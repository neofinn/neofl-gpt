...
bool HttpPost(string path,string payload,string &body,int &http){string url=BaseURL()+path;string headers="Content-Type: application/json\r\nX-NeoFL-Binding-Token: "+NeoFL_BINDING_TOKEN+"\r\n";char data[];StringToCharArray(payload,data,0,StringLen(payload),CP_UTF8);char result[];string response_headers="";ResetLastError();http=WebRequest("POST",url,headers,10000,data,result,response_headers);if(http==-1){Print("NeoFL WebRequest POST failed. Error=",GetLastError()," URL=",url);body="";return false;}body=CharArrayToString(result,0,-1,CP_UTF8);return true;}
...
bool Handshake(){if(BaseURL()==""||NeoFL_BINDING_TOKEN==""){g_authorized=false;g_execution_enabled=false;Print("NeoFL BLOCKED: API base URL and binding token are required.");return false;}string payload=StringFormat("{\"account_number\":%I64d,\"server\":\"%s\",\"connector\":\"MT5\",\"environment\":\"%s\"}",g_account,JsonEscape(g_server),EnvironmentName());string body;int http=0;if(!HttpPost("/api/handshake",payload,body,http)){g_authorized=false;g_execution_enabled=false;return false;}g_authorized=(http>=200&&http<300&&StringFind(body,"\"accepted\":true")>=0);g_execution_enabled=JsonBool(body,"execution_enabled",false);if(!g_authorized){g_execution_enabled=false;Print("NeoFL BLOCKED: handshake rejected. HTTP=",http," response=",body);}else if(!g_execution_enabled)Print("NeoFL CONNECTED: execution gate is LOCKED.");return g_authorized;}
...
bool SendTelemetry(){double balance=AccountInfoDouble(ACCOUNT_BALANCE);double equity=AccountInfoDouble(ACCOUNT_EQUITY);double profit=AccountInfoDouble(ACCOUNT_PROFIT);double drawdown=MathMax(0.0,balance-equity);int open=PositionsTotal();string payload=StringFormat("{\"account_number\":%I64d,\"server\":\"%s\",\"connector\":\"MT5\",\"environment\":\"%s\",\"balance\":%.8f,\"equity\":%.8f,\"profit\":%.8f,\"drawdown\":%.8f,\"open_trades\":%d,\"brain_status\":\"CONNECTED\"}",g_account,JsonEscape(g_server),EnvironmentName(),balance,equity,profit,drawdown,open);string body;int http=0;if(!HttpPost("/api/telemetry",payload,body,http))return false;return http>=200&&http<300;}

bool Report(string intent_id,string status,double filled,double price,string broker_order_id,string rejection){string payload=StringFormat("{\"account_number\":%I64d,\"intent_id\":\"%s\",\"adapter\":\"MT5\",\"status\":\"%s\",\"filled_quantity\":%.8f,\"average_fill_price\":%.8f,\"broker_order_id\":\"%s\",\"rejection_code\":\"%s\"}",g_account,JsonEscape(intent_id),status,filled,price,JsonEscape(broker_order_id),JsonEscape(rejection));string body;int http=0;return HttpPost("/api/v1/execution-report",payload,body,http)&&http>=200&&http<300;}
