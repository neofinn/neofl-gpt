...
bool ValidateConfiguredLotFloors()
{
   ...
   if(InpMinLotM1 < broker_min)
   {
      PrintFormat("NeoFL: InpMinLotM1 %.4f is below broker minimum %.4f.", InpMinLotM1, broker_min);
      return false;
   }

   if(InpMinLotM1 > broker_max)
   {
      PrintFormat("NeoFL: InpMinLotM1 %.4f exceeds broker maximum %.4f.", InpMinLotM1, broker_max);
      return false;
   }

   PrintFormat("NeoFL lot validation: broker min=%.4f max=%.4f step=%.4f | M5=%.4f M1=%.4f",
               broker_min, broker_max, broker_step, InpMinLotM5, InpMinLotM1);
   return true;
}
