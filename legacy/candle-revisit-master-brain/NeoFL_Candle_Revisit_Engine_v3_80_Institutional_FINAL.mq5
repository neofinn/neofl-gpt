...
#property description "NeoFL institutional engine: M5 level brain, M3 execution, M1 observation/protection, hard 0.01 lot cap, account risk governor, MT5 calendar and emergency liquidity guard."
...
// M1 is strictly an existing-trade monitor/protection/reassessment engine.
// -----------------------------------------------------------------------------


CTrade trade;

// Forward declarations: MQL5 must know these opposite/recovery helpers
// before ManageOppositeEntry() is parsed.
bool IsHedgingAccount();
bool HasOppositePosition(const bool current_buy);
bool ProfitIsFarFromTP(const bool current_buy, const double entry, const double tp, const double current);
bool ClosePositionProfitOnly(const ulong ticket);
bool OpenOppositeNormal(const bool buy);
bool OpenOppositeRecovery(const bool buy, const double loss_money, const double target_distance);
bool ManageOppositeEntry(const bool opposite_buy, const double signal_level, const bool signal_is_m1);

//--------------------------- Inputs ---------------------------------
input ENUM_TIMEFRAMES InpTimeframe          = PERIOD_M5; // M5 level/entry thesis
input ENUM_TIMEFRAMES InpExecutionTimeframe  = PERIOD_M3; // M3 level execution
input double          InpLots               = 0.01; // HARD START CAP
...
input double            InpMinLotM5             = 0.01; // HARD MIN/START LOT
...
input bool              InpEmergencyProtection   = true;
...
input double            InpMaxLot               = 0.01; // ABSOLUTE EA LOT CAP
...
input bool              InpEnableStraddle         = false; // arm only after production validation
...
input int               InpDashboardWidth        = 410;
input int               InpDashboardHeight       = 790;
input double             InpCapitalBase           = 0.0;  // 0 = lock first-run account balance as protected capital
input double             InpAccountDDWarnPct       = 8.0;  // reduce new exposure
input double             InpAccountDDHardStopPct   = 15.0; // no new exposure; emergency account lock
input double             InpSafeWithdrawalReservePct = 20.0; // separate withdrawal reserve
...
datetime g_last_exec_bar_time = 0;
...
string   g_account_risk_state = "INIT";
...
double NormalizeRequestedLots(const double requested, const double configured_min)
{
   ...
   // NeoFL absolute rule: no order generated by this EA may exceed 0.01 lot.
   const double HARD_LOT_CAP = 0.01;
   double minimum = MathMax(broker_min, configured_min);
   double maximum = MathMin(broker_max, HARD_LOT_CAP);
   if(maximum < minimum || maximum <= 0.0)
      return 0.0;

   double lots = MathMin(MathMax(requested, minimum), maximum);
...
double TradeLots()
{
   // Martingale may calculate a larger request internally, but the hard
   // account-level execution cap remains 0.01 lot.
   double lots = InpLots;
...
void DashUpdate()
{
   ...
   DashText("Brand","NeoFL",x+16,y+10,22,clrAqua,"Arial Bold");
   DashText("Tag","PRECISION. LOGIC. CONSISTENCY.",x+18,y+39,8,clrGold,"Arial Bold");
   DashText("Bot","NeoFL Candle Revisit Engine",x+16,y+54,12,clrWhite,"Arial Bold");
   DashText("Version","v3.80",x+w-58,y+56,8,clrLime,"Arial Bold");
   DashText("Desc","M5 LEVELS  |  M3 EXECUTION  |  M1 OBSERVE/PROTECT",x+16,y+74,7,clrSilver);

   DashText("MarketH","MARKET",x+16,y+104,8,clrAqua,"Arial Bold");
   DashText("Market",_Symbol+"  |  M5 levels / M3 execution",x+16,y+116,8,clrWhite);
   ...
   UpdateAccountRiskMetrics();
   double peak=g_peak_equity;
   double acctDD=(peak>0.0?100.0*(peak-eq)/peak:0.0);
   DashText("AcctH","ACCOUNT / CAPITAL",x+16,y+344,8,clrAqua,"Arial Bold");
   DashText("Bal","Balance",x+16,y+362,7,clrSilver); DashText("BalV",DashFmtMoney(bal),x+w-142,y+362,7,clrWhite,"Arial Bold");
   DashText("Eq","Equity",x+16,y+377,7,clrSilver); DashText("EqV",DashFmtMoney(eq),x+w-142,y+377,7,clrWhite,"Arial Bold");
   DashText("Peak","Peak equity",x+16,y+392,7,clrSilver); DashText("PeakV",DashFmtMoney(peak),x+w-142,y+392,7,clrWhite);
   DashText("DD","Current / Max DD",x+16,y+407,7,clrSilver); DashText("DDV",DoubleToString(acctDD,2)+"% / "+DoubleToString(g_max_drawdown_pct,2)+"%",x+w-170,y+407,7,acctDD>=InpAccountDDWarnPct?clrTomato:clrWhite,"Arial Bold");
   DashText("Risk","Account risk",x+16,y+422,7,clrSilver); DashText("RiskV",g_account_risk_state,x+w-170,y+422,7,(g_account_risk_state=="GREEN"?clrLime:clrTomato),"Arial Bold");
   DashText("FM","Free margin",x+16,y+437,7,clrSilver); DashText("FMV",DashFmtMoney(fm),x+w-142,y+437,7,clrSilver);

   DashText("RealH","REALIZED / WITHDRAWAL",x+16,y+458,8,clrAqua,"Arial Bold");
   DashText("Real","Realized profit",x+16,y+476,7,clrSilver); DashText("RealV",DashFmtMoney(g_realized_profit),x+w-142,y+476,7,g_realized_profit>=0?clrLime:clrTomato,"Arial Bold");
   DashText("Base","Protected capital",x+16,y+491,7,clrSilver); DashText("BaseV",DashFmtMoney(g_capital_base),x+w-142,y+491,7,clrWhite);
   DashText("Safe","Safe to withdraw",x+16,y+506,7,clrSilver); DashText("SafeV",DashFmtMoney(g_safe_withdrawal),x+w-142,y+506,7,g_safe_withdrawal>0?clrLime:clrGold,"Arial Bold");
   color wdClr=(StringFind(g_withdrawal_status,"SAFE")>=0?clrLime:clrGold);
   DashText("WdStatus",g_withdrawal_status,x+16,y+521,7,wdClr,"Arial Bold");

   DashText("ObsH","OBSERVER / RISK",x+16,y+546,8,clrAqua,"Arial Bold");
   DashText("Observer","Observer  "+(g_observer_ok?"ONLINE":"STALE/WAITING"),x+16,y+564,7,g_observer_ok?clrLime:clrTomato,"Arial Bold");
   DashText("ObsM1","M1  "+g_observer_m1_state+"  | MFE "+DashFmtMoney(g_observer_mfe_money),x+16,y+579,7,clrSilver);
   DashText("ObsProb","WIN "+DoubleToString(g_observer_win_prob,1)+"%  LOSS "+DoubleToString(g_observer_loss_prob,1)+"%  REC "+DoubleToString(g_observer_recovery_coverage,1)+"%",x+16,y+594,7,clrSilver);
   DashText("ObsDD","Observer DD  "+DoubleToString(g_observer_dd_pct,2)+"%  | MAE "+DashFmtMoney(g_observer_mae_money),x+16,y+609,7,clrSilver);

   DashText("GuardH","PROTECTION",x+16,y+632,8,clrAqua,"Arial Bold");
   DashText("Emergency","M1 EMERGENCY GUARD  "+(g_emergency_active?"ACTIVE":"ARMING"),x+16,y+650,7,g_emergency_active?clrGold:clrSilver,"Arial Bold");
   if(g_emergency_active)
      DashText("EmergencySL","Guard SL  "+DoubleToString(g_emergency_sl,_Digits),x+w-142,y+650,7,clrGold,"Arial Bold");
   DashText("Recovery","Opposite recovery  "+(InpAllowOppositeRecovery?"ON":"OFF"),x+16,y+666,7,InpAllowOppositeRecovery?clrLime:clrTomato,"Arial Bold");
   color monClr=(g_m1_monitor_state=="SCANNING" || g_m1_monitor_state=="MONITORING")?clrLime:clrSilver;
   DashText("M1Mon","M1 monitor  "+g_m1_monitor_state,x+16,y+682,7,monClr,"Arial Bold");

   UpdateCalendarGovernor(false);
   color newsClr=g_calendar_locked?clrTomato:clrLime;
   DashText("News","EVENT GOVERNOR  "+(g_calendar_locked?"BLOCKED":"CLEAR"),x+16,y+706,7,newsClr,"Arial Bold");
   DashText("NewsRule","NEW EXPOSURE BLOCK  4H BEFORE  |  30M AFTER",x+16,y+721,7,clrGold);
   DashText("Footer","NeoFL Trading Systems",x+16,y+744,7,clrAqua,"Arial Bold");
}
...
string AccountRiskKey()
{
   return "NEOFL_PEAK_EQUITY_"+(string)AccountInfoInteger(ACCOUNT_LOGIN);
}

void UpdateAccountRiskMetrics()
{
   double eq=AccountInfoDouble(ACCOUNT_EQUITY);
   if(eq<=0.0) return;

   string key=AccountRiskKey();
   if(GlobalVariableCheck(key))
      g_peak_equity=GlobalVariableGet(key);
   else
   {
      g_peak_equity=eq;
      GlobalVariableSet(key,g_peak_equity);
   }
   ...
   double dd=(g_peak_equity>0.0 ? 100.0*(g_peak_equity-eq)/g_peak_equity : 0.0);
   dd=MathMax(0.0,dd);
   g_max_drawdown_pct=MathMax(g_max_drawdown_pct,dd);

   if(dd>=InpAccountDDHardStopPct)
      g_account_risk_state="RED / HARD STOP";
   else if(dd>=InpAccountDDWarnPct)
      g_account_risk_state="AMBER / REDUCE";
   else
      g_account_risk_state="GREEN";
}
...
void UpdateWithdrawalMetrics()
{
   ...
   double reserve=eq*MathMax(0.0,MathMin(100.0,InpSafeWithdrawalReservePct))/100.0;
...
bool PreTradeRiskGate(const bool recovery,const double lots,const string context)
{
   ...
   if(g_account_risk_state=="RED / HARD STOP")
   {
      g_risk_gate_state="LOCKED";
      g_risk_gate_reason="ACCOUNT DD HARD STOP";
      return false;
   }

   if(g_account_risk_state=="AMBER / REDUCE" && !recovery)
   {
      g_risk_gate_state="LOCKED";
      g_risk_gate_reason="ACCOUNT DD REDUCE";
      return false;
   }
   ...
   {
      // Institutional rule: the calendar governor blocks ALL NEW EXPOSURE.
      // Existing positions remain under the continuous monitoring engine.
      g_risk_gate_state="LOCKED";
...
double FundConstrainedLots(const bool recovery,const double requested)
{
   const double HARD_LOT_CAP=0.01;
   double broker_min=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MIN);
   double broker_max=MathMin(SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_MAX),HARD_LOT_CAP);
   double step=SymbolInfoDouble(_Symbol,SYMBOL_VOLUME_STEP);
   if(step<=0.0) step=broker_min;

   if(broker_max<broker_min || broker_max<=0.0)
      return 0.0;

   double capped_request=MathMin(requested,HARD_LOT_CAP);
   if(!InpUseFundEngine)
      return NormalizeRequestedLots(capped_request,InpMinLotM5);
   ...
   double budget=recovery ? g_fund_recovery_budget : g_fund_m5_budget;
   double max_by_margin=MathMax(0.0,budget)/margin_one;
   double lots=MathMin(capped_request,MathMin(broker_max,max_by_margin));
   if(lots<broker_min) return 0.0;
   lots=MathFloor(lots/step+1e-9)*step;
   lots=MathMin(lots,HARD_LOT_CAP);
   if(lots<broker_min) return 0.0;
   return NormalizeDouble(lots,2);
}
...
bool ExecuteSignal(const Level &L, const bool buy)
{
   ...
   datetime bar_time = iTime(_Symbol, InpExecutionTimeframe, 1);
...
//+------------------------------------------------------------------+
//| Opposite/recovery helpers                                        |
//| These functions deliberately enforce the NeoFL 0.01 hard cap.   |
//+------------------------------------------------------------------+
bool IsHedgingAccount()
{
...
bool HasOppositePosition(const bool current_buy)
{
   for(int i=PositionsTotal()-1;i>=0;--i)
   {
      ...
      if(pos_buy!=current_buy)
         return true;
   }
   return false;
}

bool ProfitIsFarFromTP(const bool current_buy,
                       const double entry,
                       const double tp,
                       const double current)
{
   if(entry<=0.0 || tp<=0.0 || current<=0.0)
      return false;

   double original_distance=MathAbs(tp-entry);
   if(original_distance<=0.0)
      return false;

   // For a BUY, price must still be below TP. For a SELL, above TP.
   if(current_buy && current>=tp)
      return false;
   if(!current_buy && current<=tp)
      return false;

   double remaining=MathAbs(tp-current);
   double remaining_ratio=remaining/original_distance;
   return (remaining_ratio>=InpProfitFarFromTPPct);
}
...
bool ClosePositionProfitOnly(const ulong ticket)
{
   ...
   if(!ok)
   {
      PrintFormat("NeoFL profit-only close failed ticket=%I64u profit=%.2f ret=%d %s",
                  ticket,profit,trade.ResultRetcode(),trade.ResultRetcodeDescription());
      return false;
   }

   PrintFormat("NeoFL profit-only close: ticket=%I64u realized=%.2f",ticket,profit);
   return true;
}

bool OpenOppositeNormal(const bool buy)
{
   double requested=TradeLots()*MathMax(0.0,InpOppositeNormalLotFactor);
   double lots=FundConstrainedLots(true,requested);
   if(lots<=0.0)
   {
      Print("NeoFL opposite normal skipped: fund/0.01 hard-cap constraint.");
      return false;
   }

   if(!PreTradeRiskGate(true,lots,"M1 OPPOSITE NORMAL"))
      return false;

   double atr=GetATR(PERIOD_M3,InpM5ATRPeriod);
   if(atr<=0.0)
      atr=GetATR(PERIOD_M5,InpM5ATRPeriod);
   if(atr<=0.0)
      return false;
   ...
   double tp=buy ? entry+atr*InpRecoveryTargetATR
                 : entry-atr*InpRecoveryTargetATR;
   tp=NormalizePrice(tp);
   ...
   if(!ok)
      PrintFormat("NeoFL opposite normal failed ret=%d %s",
                  trade.ResultRetcode(),trade.ResultRetcodeDescription());
   return ok;
}

bool OpenOppositeRecovery(const bool buy,
                          const double loss_money,
                          const double target_distance)
{
   if(loss_money<=0.0 || target_distance<=0.0)
      return false;

   // Recovery is calculated from projected profit per 1 lot, then capped.
   double ask=SymbolInfoDouble(_Symbol,SYMBOL_ASK);
   double bid=SymbolInfoDouble(_Symbol,SYMBOL_BID);
   double entry=buy ? ask : bid;
   if(entry<=0.0)
   ...
   double target=buy ? entry+target_distance : entry-target_distance;
   target=NormalizePrice(target);

   double profit_one_lot=0.0;
   ENUM_ORDER_TYPE order_type=buy ? ORDER_TYPE_BUY : ORDER_TYPE_SELL;
   if(!OrderCalcProfit(order_type,_Symbol,1.0,entry,target,profit_one_lot))
   {
      PrintFormat("NeoFL opposite recovery: OrderCalcProfit failed err=%d",GetLastError());
      return false;
   }

   profit_one_lot=MathAbs(profit_one_lot);
   if(profit_one_lot<=0.0)
      return false;

   double requested=loss_money/profit_one_lot;
   double lots=FundConstrainedLots(true,requested);
   if(lots<=0.0)
   {
      PrintFormat("NeoFL opposite recovery skipped: required %.4f lots exceeds broker/fund/0.01 constraints.",requested);
      return false;
   }

   if(!PreTradeRiskGate(true,lots,"M1 OPPOSITE RECOVERY"))
      return false;
   ...
   bool ok=buy
         ? trade.Buy(lots,_Symbol,0.0,0.0,target,"CLVL Opposite Recovery")
         : trade.Sell(lots,_Symbol,0.0,0.0,target,"CLVL Opposite Recovery");

   if(!ok)
      PrintFormat("NeoFL opposite recovery failed ret=%d %s",
                  trade.ResultRetcode(),trade.ResultRetcodeDescription());
   else
      PrintFormat("NeoFL opposite recovery opened: lots=%.2f loss=%.2f target=%.5f",
                  lots,loss_money,target);
   return ok;
}
...
bool ValidateConfiguredLotFloors()
{
   ...
   if(broker_min > 0.01 + 1e-9)
   {
      PrintFormat("NeoFL: HARD 0.01 lot cap cannot be satisfied; broker minimum is %.4f.", broker_min);
      return false;
   }

   if(InpLots > 0.01 || InpMinLotM5 > 0.01 || InpMaxLot > 0.01)
   {
      Print("NeoFL: HARD LOT CAP violation. InpLots/MinLot/MaxLot must all be <= 0.01.");
      return false;
   }
...
void UpdateTesterObserverBridge()
{
   if(!InpTesterObserverBridge || !MQLInfoInteger(MQL_TESTER))
      return;

   NeoFLObs_Update(InpObserverPrefix,_Symbol,InpMagic,
                   InpObserverM1ConfirmBars,
                   InpObserverProfitArmMoney,
                   InpObserverRetracePct,
                   InpObserverATRReversalMult,
                   InpObserverProbabilityTrades);
}
...
bool ReadObserverNetwork()
{
   ...
   double hb=ReadObserverValue("HEARTBEAT",0.0);
   datetime now=TimeCurrent();

   if(hb<=0.0 || (InpObserverStaleSeconds>0 && now-(datetime)hb>InpObserverStaleSeconds))
   {
      g_observer_ok=false;
      g_observer_m1_state="STALE";
      if(InpObserverFailClosed)
         g_risk_gate_reason="OBSERVER STALE";
      return false;
   }

   g_observer_ok=true;
   g_observer_update=(datetime)hb;
   ...
   else g_observer_m1_state="UNKNOWN";
...
int OnInit()
{
   ...
   g_last_exec_bar_time = iTime(_Symbol, InpExecutionTimeframe, 0);
   UpdateAccountRiskMetrics();

   Print("NeoFL v3.80 initialized: M5 levels, M3 execution, M1 observation/protection, hard 0.01 lot cap.");
...
void OnTick()
{
   ...
   if(SelectOurPosition(ticket))
   {
      ...
      // M1 observer is informational only for normal trade management.
      // It must NOT prematurely close a profitable trade. Emergency closure
      // is handled exclusively by the tick-level liquidity guard above.
      if(g_observer_ok && g_observer_exit)
      {
         Print("NeoFL M1 observer: adverse condition observed; normal exit suppressed. Emergency guard remains authoritative.");
      }

      // Observer-driven opposite thesis. Execution remains in the main EA.
      if(g_observer_ok && g_observer_opposite)
      {
         bool opposite_buy=!buy;
         double level=current;
         if(ManageOppositeEntry(opposite_buy,level,true))
         {
            g_observer_opposite=false;
            GlobalVariableSet(ObserverKey("M1_OPPOSITE"),0.0);
         }
      }
   }
   ...
   // M5 creates/refreshes the structural levels.
   datetime m5bar=iTime(_Symbol,InpTimeframe,0);
   if(m5bar>0 && m5bar!=g_last_bar_time)
   {
      g_last_bar_time=m5bar;
      MqlRates m5rates[];
      ArrayResize(m5rates,3);
      ArraySetAsSeries(m5rates,true);
      if(CopyRates(_Symbol,InpTimeframe,0,3,m5rates)>=3)
         ClassifyAndCreateLevels(m5rates[1],1);
   }

   // M3 is the ONLY level execution timeframe. M1 never executes.
   datetime m3bar=iTime(_Symbol,InpExecutionTimeframe,0);
   if(m3bar<=0 || m3bar==g_last_exec_bar_time)
      return;
   g_last_exec_bar_time=m3bar;

   MqlRates m3rates[];
   ArrayResize(m3rates,3);
   ArraySetAsSeries(m3rates,true);
   if(CopyRates(_Symbol,InpExecutionTimeframe,0,3,m3rates)<3)
      return;

   EvaluateSignals(m3rates[1],m3rates[2]);
}
