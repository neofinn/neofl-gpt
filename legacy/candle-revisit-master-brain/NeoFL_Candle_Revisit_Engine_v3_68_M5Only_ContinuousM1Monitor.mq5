...
#property description "Standalone M5 candle-level entry engine with continuous M1 monitoring, profit-only management, and opposite recovery."
...
input int               InpDashboardWidth        = 390;
input int               InpDashboardHeight       = 570;
...
void DashUpdate()
{
   ...
   // Brand block: fixed alignment so the bot name/version never collide.
   DashText("Brand","NeoFL",x+20,y+16,28,clrAqua,"Arial Bold");
   DashText("Tag","PRECISION. LOGIC. CONSISTENCY.",x+22,y+48,9,clrGold,"Arial Bold");
   DashText("Bot","NeoFL Candle Revisit Engine",x+20,y+70,15,clrWhite,"Arial Bold");
   DashText("Version","v3.68",x+w-78,y+94,11,clrLime,"Arial Bold");
   DashText("Desc","M5 ENTRY ENGINE  •  M1 MONITOR/PROTECTION  •  CTrade",x+20,y+96,9,clrSilver);

   DashText("MarketH","MARKET",x+20,y+124,10,clrAqua,"Arial Bold");
   DashText("Market",_Symbol+" • M5 entry / M1 monitor",x+20,y+142,11,clrWhite);
   DashText("Bid","Bid  "+DoubleToString(bid,_Digits),x+20,y+162,10,clrSilver);
   DashText("Ask","Ask  "+DoubleToString(ask,_Digits),x+w/2,y+162,10,clrSilver);

   DashText("RegimeH","REGIME",x+20,y+190,10,clrAqua,"Arial Bold");
   DashText("Regime",regime,x+w-110,y+188,11,regimeClr,"Arial Bold");
   DashText("ATR","ATR("+IntegerToString(InpM5ATRPeriod)+")  "+DoubleToString(atr,_Digits),x+20,y+210,10,clrWhite);

   DashText("ScanH","LEVEL ENGINE",x+20,y+238,10,clrAqua,"Arial Bold");
   DashText("Scan",scan,x+w-85,y+236,11,scanClr,"Arial Bold");
   DashText("Levels","Open levels  "+IntegerToString(active),x+20,y+258,10,clrWhite);
   DashText("Revisit","Revisited     "+IntegerToString(revisited),x+20,y+276,10,clrSilver);

   DashText("PosH","POSITION",x+20,y+304,10,clrAqua,"Arial Bold");
   color pClr=(p>0 ? clrLime : (p<0 ? clrTomato : clrSilver));
   DashText("Pos",pos,x+w-85,y+302,11,pClr,"Arial Bold");
   DashText("Float","Floating P/L  "+DashFmtMoney(p),x+20,y+324,10,pClr);
   DashText("Lots","Lots           "+DoubleToString(lots,2),x+20,y+342,10,clrWhite);
   if(lots>0)
   {
      DashText("Entry","Entry          "+DoubleToString(entry,_Digits),x+20,y+360,10,clrSilver);
      DashText("TP","TP             "+(tp>0?DoubleToString(tp,_Digits):"MONITORED"),x+20,y+378,10,clrSilver);
   }

   DashText("AcctH","ACCOUNT",x+20,y+406,10,clrAqua,"Arial Bold");
   DashText("Bal","Balance        "+DashFmtMoney(bal),x+20,y+426,10,clrWhite);
   DashText("Eq","Equity         "+DashFmtMoney(eq),x+20,y+444,10,clrWhite);
   DashText("FM","Free Margin    "+DashFmtMoney(fm),x+20,y+462,10,clrSilver);

   DashText("Rules","NO INITIAL SL  •  PROFIT-ONLY EARLY EXIT",x+20,y+490,9,clrGold,"Arial Bold");
   DashText("Recovery","OPPOSITE RECOVERY: "+(InpAllowOppositeRecovery?"ON":"OFF"),
            x+20,y+508,8,InpAllowOppositeRecovery?clrLime:clrTomato,"Arial Bold");

   color monClr = (g_m1_monitor_state=="SCANNING" || g_m1_monitor_state=="MONITORING")
                  ? clrLime : clrSilver;
   string lastScan = (g_m1_last_closed_scanned>0)
                   ? TimeToString(g_m1_last_closed_scanned,TIME_MINUTES|TIME_SECONDS)
                   : "--";
   DashText("M1Mon","M1 MONITOR: "+g_m1_monitor_state,
            x+20,y+526,8,monClr,"Arial Bold");
   DashText("M1Scan","M1 CLOSED SCANS: "+(string)g_m1_scan_count+"  •  LAST: "+lastScan,
            x+20,y+544,8,clrSilver);
   DashText("Footer","NeoFL Trading Systems",x+20,y+562,11,clrAqua,"Arial Bold");
                           ...
                           const double target_distance)
{
   ...
   // Recovery positions have NO broker TP. The recovery basket is closed
   // only by the continuous basket monitor once combined P/L reaches the
   // configured recovery target. This prevents the hedge TP from closing
   // alone and leaving the original trade stranded in loss.
   trade.SetExpertMagicNumber(InpMagic);
   trade.SetDeviationInPoints(InpDeviationPoints);
   trade.SetTypeFilling(InpFillingMode);

   bool ok = opposite_buy
           ? trade.Buy(lots, _Symbol, 0.0, 0.0, 0.0, "CLVL Opposite Recovery")
           : trade.Sell(lots, _Symbol, 0.0, 0.0, 0.0, "CLVL Opposite Recovery");
   ...
   PrintFormat("Opposite recovery opened: %s %.2f lots, target %.2f, loss covered %.2f",
               opposite_buy ? "BUY" : "SELL", lots, target_distance, current_loss);
   return true;
}
...
{
   // CONTINUOUS EXISTING-TRADE MONITOR
   // M1 only observes and manages an already-open trade.
   // No initial SL is placed. Voluntary closure is allowed only when the
   ...
   if(!SelectOurPosition(ticket))
   {
      ...
      g_m1_monitor_state="IDLE";
      return;
   }

   g_m1_monitor_state="MONITORING";
   ...
   // Reset internal monitor state when a new ticket appears.
   if(g_trailing_ticket != ticket)
   {
   ...
   // Internal trailing has NO broker SL. It only records the favorable
   // extreme and may request a profit-only close after a pullback.
   if(buy)
      g_trailing_extreme=MathMax(g_trailing_extreme,current);
   else
      g_trailing_extreme=(g_trailing_extreme<=0.0
                          ? current
                          : MathMin(g_trailing_extreme,current));

   // Do not arm the trailing monitor until the requested number of closed
   // M1 candles has elapsed since entry.
   datetime entry_time=(datetime)PositionGetInteger(POSITION_TIME);
   int elapsed_bars=0;
   datetime closed_time=iTime(_Symbol,PERIOD_M1,1);
   if(entry_time>0 && closed_time>entry_time)
      elapsed_bars=(int)((closed_time-entry_time)/60);

   if(InpUseM1Trailing && elapsed_bars>=InpTrailingStartBarsM1 && profit>0.0)
      g_trailing_active=true;

   if(!g_trailing_active || profit<=0.0)
      return;

   // Use the configured distance, but make it volatility-aware.
   double point=PointValue();
   double atr=GetATR(PERIOD_M1,InpM5ATRPeriod);
   double trail_distance=InpTrailingDistancePts*point;
   if(atr>0.0 && InpM1Trail_ATR_Mult>0.0)
      trail_distance=MathMax(trail_distance,atr*InpM1Trail_ATR_Mult);

   if(trail_distance<=0.0)
      return;

   bool pullback = buy
                 ? (current <= g_trailing_extreme-trail_distance)
                 : (current >= g_trailing_extreme+trail_distance);

   if(!pullback)
      return;

   // Pullback exit is profit-only and happens from the live monitor.
   // Mark the reassessment window before closing so the next closed M1
   // candle can reassess the original M5 opportunity.
   if(InpReassessAfterTrail)
   {
      g_m1_reassess_pending=true;
...
void ReassessTrailingSetupM1(const MqlRates &bar)
{
   ...
   // existing setup on M1. Do NOT immediately reassess the M5 level.
...
{
   if(!InpAllowM1OppositeProtection)
      return false;
...
int OnInit()
{
   ...
   Print("NeoFL Candle Revisit Engine v3.68 initialized. M5 entry engine only; M1 continuous monitor/protection.");
   DashUpdate();
...
void OnTick()
{
   ...
   // Continuous protection: recovery basket first, then normal M1 monitoring.
   ...
   if(m1_bar != 0 && m1_bar != g_m1_last_bar)
   {
      ...
      {
         // ONLY the last CLOSED M1 candle is analysed.
         // The running M1 candle is never used for a signal decision.
         g_m1_last_closed_scanned=m1rates[1].time;
         g_m1_scan_count++;
         g_m1_monitor_state=HasOpenPosition() ? "SCANNING" : "WAITING";

         if(HasOpenPosition())
         {
            ReassessTrailingSetupM1(m1rates[1]);

            // M1 may recognize an opposite setup only as protection for an
            // already-open M5 trade. It can NEVER create the initial trade.
            ulong active_ticket=0;
            if(SelectOurPosition(active_ticket))
            {
               bool current_buy=(PositionGetInteger(POSITION_TYPE)==POSITION_TYPE_BUY);
               bool opposite_buy=false;
               if(FindM1OppositeSignal(current_buy,opposite_buy))
               {
                  g_m1_last_opposite_signal=m1rates[1].time;
                  ManageOppositeEntry(opposite_buy,m1rates[1].close,true);
               }
            }
   ...
   // M5 opposite setup has priority when an M5-origin trade is already open.
   ulong active_ticket=0;
   if(SelectOurPosition(active_ticket))
   {
      bool current_buy=(PositionGetInteger(POSITION_TYPE)==POSITION_TYPE_BUY);
      bool opposite_buy=false;
      double opposite_level=0.0;
      if(FindM5OppositeSignal(current_buy,rates[1],rates[2],opposite_buy,opposite_level))
         ManageOppositeEntry(opposite_buy,opposite_level,false);
   }
