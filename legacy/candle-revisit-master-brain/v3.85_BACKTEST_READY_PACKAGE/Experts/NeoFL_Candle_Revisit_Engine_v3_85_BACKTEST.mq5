...
#property description "NeoFL v3.85 backtest EA: Master Brain, Observer, Straddle, Calendar and Fund-Risk run internally; CTrade is the only execution authority."
...
input bool              InpUseExternalDataFeeder     = false;  // LIVE: external script supplies Observer/Fund/Calendar state
...
input bool              InpMasterBrainEnabled   = true;
input double            InpMasterDesiredBasketProfitMoney = 1.00;
input double            InpMasterBasketProfitLockMoney = 1.00;
input double            InpMasterEmergencyBasketLossPct = 5.0;
...
void UpdateFundSnapshot()
{
   if(ExternalFeederActive())
   {
      ...
      g_fund_free_margin       = ReadFeederValue("FREE_MARGIN",AccountInfoDouble(ACCOUNT_MARGIN_FREE));
...
{
   if(!InpMasterBrainEnabled) return false;
...
bool ExecuteObserverStraddleState()
{
   ...
   if(g_observer_straddle_arm)
   {
      ...
      if(SelectOurPosition(main_ticket) && PositionSelectByTicket(main_ticket))
      {
         ...
         if(OpenStraddle(straddle_buy,g_observer_straddle_lots,px))
         {
...
{
   if(!InpMasterBrainEnabled) return;
   ...
   c.desired_basket_profit=InpMasterDesiredBasketProfitMoney;
   c.profit_lock_money=InpMasterBasketProfitLockMoney;
   c.emergency_basket_loss_pct=InpMasterEmergencyBasketLossPct;
   c.probability_trades=InpObserverProbabilityTrades;
   NeoFLMB_Update(InpObserverPrefix,_Symbol,InpMagic,c);
}
