...
//|                                                                  |
//| NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING - Dedicated Gold Active Scanner                     |
//|                                                                  |
//| PURPOSE                                                          |
...
//+------------------------------------------------------------------+
// ================================================================
// SHORT-TERM INTRADAY PRIORITY PROFILE
// M5/M15 price action + fast EMA/RSI/MACD/ADX carry the trend decision.
// M30 is context/survival only; H1/H4 never outrank the intraday engine.
// ================================================================
// ================================================================
// NeoFL GOLD 6.2 — UNIFIED DUAL-ENGINE ARCHITECTURE
// Trend and ARK scan independently.
// ARK event -> Trend PAUSES -> ARK manages its trade -> Trend resumes.
// No engine-conflict arbitration and no forced trade.
// ================================================================
input bool   GoldArkPausesTrend = true;
input bool   GoldTrendRequiresM5SyntheticM15 = true;
input bool   GoldUseM30SurvivalGate = true;
input bool   GoldUseSupplyDemandConfluence = true;
input bool   GoldUseSMCConfluence = true;
input bool   GoldUseM1LiquidityForEntry = true;
input bool   GoldUseM1Trailing = true;
input bool   GoldUseRewardAwareSizing = true;
// M1 IS NOT A TREND VOTE: it is reserved for liquidity sweep, entry timing,
// stop management and trailing. Trend direction is M5 + synthetic M15.
//
#property strict
...
#property description "NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING - Standalone GOLD Trading Bot"
...
// NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING: GOLD uses the dedicated MT5 account as its full strategy pool. No cross-bot capital sharing.
...
input int TrendRSIPeriod = 9;
input int TrendADXPeriod = 10;
input int TrendMACDFast = 8;
input int TrendMACDSlow = 21;
input double TrendEMAStackWeight = 0.20;
input double TrendRSIWeight = 0.15;
input double TrendMACDWeight = 0.15;
input double TrendADXWeight = 0.15;
...
// Price-action confirmation. A short run of decisive M5 candles should
// materially improve the M5 trend read; it does not replace M15 confirmation.
input bool   GoldUseM5CandlePressure = true;
input int    GoldM5ConsecutiveCandleCount = 3;
input double GoldM5MinBodyATR = 0.15;
input double GoldM5CandlePressureWeight = 0.25;
input bool   GoldUseM15CandlePressure = true;
...
// It runs beside the normal NeoFL trend engine and is an M15 event overlay; it does not replace the normal indicator engine.
input bool   GoldArkStyleEngineEnabled = true;
input ENUM_TIMEFRAMES GoldArkTimeframe = PERIOD_M15;
input bool   GoldArkEventDriven = true;
// Execution policy: Trend owns normal market flow.
// ARK is an M15 event overlay: it boosts same-direction Trend setups,
// but never opens an opposite-direction trade against an active Trend.
input bool   GoldTrendHasPriority = false; // Deprecated: ARK has priority when active.
// Trend confirmation policy:
// M5 + M15 must agree before a normal Trend trade.
// M30 is a survival/context check: strong opposition pauses new Trend entries
// for reevaluation rather than forcing a reversal.
input bool   GoldRequireM5M15Confirmation = true;
input double GoldM30SurvivalThreshold = 0.35;
input int    GoldTradeCadenceMinutes = 15;
// This is a cadence TARGET, not a forced-trade guarantee. A trade is allowed
// only when a valid setup and the local risk gate exist.

input bool   GoldTrendIndependentFromSMC = true;

input double GoldArkEventMinScore = 0.60;
input int    GoldArkEventCooldownMinutes = 20;
...
input double GoldM5ReversalTrendWeight = 0.55;
input double GoldM15ConfirmWeight = 0.30;
input double GoldHTFContextWeight = 0.05;
   ...
   double atr)
{
   ...
   // Intraday priority: fast EMA/slope + RSI + MACD + ADX dominate.
   // EMA50/200 remains a secondary filter, not the primary intraday driver.
   double intradayCore=core*0.55+
                        rsiScore*TrendRSIWeight+
                        macdScore*TrendMACDWeight+
                        adxScore*TrendADXWeight;

   double totalWeight=0.55+TrendEMAStackWeight+TrendRSIWeight+
                      TrendMACDWeight+TrendADXWeight;

   return Clamp(
      (intradayCore+
       stack*TrendEMAStackWeight)/totalWeight,
      -1.0,1.0);
}
   ...
   int aliasCount)
{
   ...
   {
      Print(
         "NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | AUTO MAP | ",
         canonical,
         " -> NOT FOUND"
      );
      return;
...
void BuildAutoDetectedUniverse()
{
   ...
   Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | DEDICATED ACTIVE SCAN | GOLD symbols=",MarketCount);
   if(GoldSymbol!="")
      Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | ACTIVE SYMBOL=",GoldSymbol);
   else
      Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | GOLD/XAU SYMBOL NOT FOUND");
}
...
void BuildUniverse()
{
   ...
   for(int i=0;i<n;i++)
   {
      ...
      if(requested=="") continue;       if(!IsGoldSymbolName(requested))
       {
          Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | MANUAL SYMBOL REJECTED (NOT GOLD): ",requested);
         continue;
      }
   ...
   Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | MANUAL GOLD UNIVERSE=",MarketCount);
}
...
void ScanMarkets()
{
   ...
   for(int i=0;i<MarketCount;i++)
   {
      ...
      {
         if(PrintSymbolMapping)
            Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | SCAN REJECTED NON-GOLD: ",Symbols[i]);
         continue;
      }
...
void PrintTopMarkets()
{
   ...
   Print(
      "================ NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING ACTIVE OPPORTUNITY SCAN ================"
   );
   ...
   {
      Print(
         "NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | BEST OPPORTUNITY = ",
   ...
   {
      Print(
         "NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | NO QUALIFIED OPPORTUNITY"
      );
   }
...
void DisplayDashboard()
{
   ...
   string text=
      "NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING - STANDALONE ACTIVE SCANNER\n";
   text+=
      "GLOBAL SESSION: "+
      currentSession+
      "\n";
...
void CancelOurPending(string symbol="")
{
   for(int i=OrdersTotal()-1;i>=0;i--)
   {
      ...
      if(ot==ORDER_TYPE_BUY_STOP || ot==ORDER_TYPE_SELL_STOP)
      {
         ...
         if(PrintExecutionLog) Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | CANCEL PENDING | ",os," ticket=",ticket);
      }
   }
                           ...
                           double entry)
{
   ...
   if(PrintRiskManagerLog)
      Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING CAPITAL | DEDICATED ACCOUNT",
                                  ...
                                  double entry2)
{
   ...
   if(PrintRiskManagerLog)
      Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING CAPITAL | DEDICATED COMBINED",
            " equity=",DoubleToString(accountEquity,2),
            " usable=",DoubleToString(usable,2),
            " required=",DoubleToString(required,2),
            " free=",DoubleToString(accountFree,2),
...
void CancelPendingAfterFill(string symbol)
{
   ...
   if(symbol==GoldSymbol && GoldNewsActive && GoldNewsStraddleArmed)
   {
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | NEWS RIDER | opposite stop retained");
      return;
   }
         ...
         ot==ORDER_TYPE_BUY_LIMIT || ot==ORDER_TYPE_SELL_LIMIT)
      {
         if(!DryRun)
         {
            if(!trade.OrderDelete(ticket) && PrintExecutionLog)
               Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | OCO DELETE FAILED | ",symbol,
                     " ticket=",ticket," retcode=",trade.ResultRetcode(),
                     " ",trade.ResultRetcodeDescription());
         }
         else if(PrintExecutionLog)
            Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | DRY OCO DELETE | ",symbol," ticket=",ticket);
      }
   }
...
void ProtectGoldPositionOnLiquidityShock(int shockDirection,double atrPrice)
{
   ...
   if(pt==POSITION_TYPE_BUY && shockDirection<0 && bid>0)
   {
      ...
      if(oldSL<=0 || newSL>oldSL)
      {
         ...
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | SHOCK PROTECT BUY | SL ",
   ...
   if(pt==POSITION_TYPE_SELL && shockDirection>0 && ask>0)
   {
      ...
      if(oldSL<=0 || newSL<oldSL)
      {
         if(!DryRun) trade.PositionModify(GoldSymbol,newSL,tp);
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | SHOCK PROTECT SELL | SL ",
                  DoubleToString(oldSL,DigitsOf(GoldSymbol))," -> ",
                  DoubleToString(newSL,DigitsOf(GoldSymbol)));
      }
   }
...
bool PlaceGoldNewsStraddle()
{
   ...
   if(a && b)
   {
      ...
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | NEWS STRADDLE ARMED | ",GoldNewsEventName,
...
void TrailGoldNewsRider()
{
   ...
   else if(dir!=GoldNewsPositionDirection)
   {
      ...
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | NEWS RIDER REVERSAL | direction=",dir,
   ...
   for(int i=OrdersTotal()-1;i>=0;i--)
   {
      ...
      if(PrintExecutionLog && moved)
         Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | NEWS RIDER TRAIL | posSL=",
               DoubleToString(newSL,DigitsOf(GoldSymbol)),
...
void UpdateGoldNewsEngine()
{
   ...
   if(GoldNewsStraddleArmed)
   {
      ...
      if(!PositionSelect(GoldSymbol))
      {
         ...
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | NEWS CAMPAIGN COMPLETE | recovery lock started");
      }
   }
}
...
void UpdateGoldLiquidityDefense()
{
   ...
   if(GoldShockActive)
   {
      if(TimeCurrent()>=GoldShockUntil && GoldShockConfirmationComplete())
      {
         ...
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | SHOCK LOCK CLEARED | scanning resumed");
      }
      return;
   }
   ...
   if(PrintExecutionLog)
      Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | LIQUIDITY SWEEP | ",
...
bool GoldArkEventIsActive(double arkScore,int arkDirection,datetime eventTime)
{
   if(!GoldArkStyleEngineEnabled || !GoldArkEventDriven) return false;
   if(arkDirection==0) return false;
   if(arkScore<GoldArkEventMinScore) return false;

   static datetime lastArkEventTime=0;
   if(eventTime<=0) return false;

   // Prevent repeated execution from the same M15 structural event.
   if(lastArkEventTime>0 &&
      eventTime<=lastArkEventTime) return false;

   if(GoldArkEventCooldownMinutes>0 &&
      lastArkEventTime>0 &&
      (eventTime-lastArkEventTime)<GoldArkEventCooldownMinutes*60)
      return false;

   lastArkEventTime=eventTime;
   return true;
}

// ARK EVENT OVERLAY — M15 structural events are optional and never a mandatory Trend gate.
bool PlaceGoldTrendPending(MARKET_STATE &m)
{
   ...
   else
   {
      ...
      // gate. M5 is confluence only and cannot veto an ARK setup.
   }
   ...
   {
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | SMC REQUIRED BLOCK | dir=",tradeDirection,
   ...
   {
      if(PrintRiskManagerLog)
         Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING CAPITAL | GOLD ORDER BLOCKED | bucket limit");
      return false;
   }

   bool ok=false;
   if(PrintExecutionLog)
      Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | SMC TREND | dir=",tradeDirection,
   ...
   if(!DryRun)
   {
      ...
      if(!ok && PrintExecutionLog)
         Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | ORDER FAILED | retcode=",
...
bool PlaceGoldSidewaysStraddle(MARKET_STATE &m)
{
   ...
   double lots=CalculateLots(GoldSymbol,slDist/point);    if(!AccountCapitalAllowsCombined(GoldSymbol,ORDER_TYPE_BUY_STOP,lots,buy,GoldSymbol,ORDER_TYPE_SELL_STOP,lots,sell))
    {
       if(PrintRiskManagerLog)
          Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING CAPITAL | GOLD RANGE BLOCKED | bucket limit");
       return false;
    }

    bool ok1=true,ok2=true;
   if(PrintExecutionLog)
      Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | GOLD SIDEWAYS STRADDLE | ",GoldSymbol," buy=",DoubleToString(buy,DigitsOf(GoldSymbol))," sell=",DoubleToString(sell,DigitsOf(GoldSymbol)));
   if(!DryRun)
   {
             trade.SetExpertMagicNumber(BotMagicNumber);
ok1=trade.BuyStop(lots,buy,GoldSymbol,buySL,buyTP,ORDER_TIME_GTC,0,"NeoFL4.2 GOLD RANGE BUY");
      if(!ok1 && PrintExecutionLog) Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | GOLD BUY STOP FAILED | ",trade.ResultRetcode()," ",trade.ResultRetcodeDescription());
      ok2=trade.SellStop(lots,sell,GoldSymbol,sellSL,sellTP,ORDER_TIME_GTC,0,"NeoFL4.2 GOLD RANGE SELL");
      if(!ok2 && PrintExecutionLog) Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | GOLD SELL STOP FAILED | ",trade.ResultRetcode()," ",trade.ResultRetcodeDescription());
   }
   if(ok1||ok2)
   {
      GoldPendingSymbol=GoldSymbol;
...
bool DetectGoldArkFreshZone(int direction,GOLD_ARK_SIGNAL &a)
{
   ...
   int copied=CopyRates(GoldSymbol,GoldArkTimeframe,1,need,r);
...
bool EvaluateGoldArkStyle(GOLD_ARK_SIGNAL &a,MARKET_STATE &m)
{
   ...
   // ARK is an event-driven structural overlay.
   // Its primary structure timeframe is M15 by default.
   datetime arkBar=iTime(GoldSymbol,GoldArkTimeframe,1);
   if(arkBar<=0) return false;

   static datetime lastScannedArkBar=0;
   static datetime lastConsumedArkEvent=0;

   // Do not repeatedly re-score the same M15 structural event every 2 seconds.
   if(lastScannedArkBar==arkBar) return false;
   lastScannedArkBar=arkBar;
   ...
   for(int k=0;k<2;k++)
   {
      ...
      // M15 is the ARK structural direction; M5 remains the normal Trend engine.
      double arkTFScore=Clamp(dir*m.tf[2].trend,0.0,1.0);
      ...
      c.score=
         ...
         arkTFScore*GoldArkM5Weight;

      if(arkTFScore>=GoldM5ReversalMinScore)
   ...
   if(!bestSignal.valid || best<MathMax(GoldArkMinScore,GoldArkEventMinScore))
      return false;

   // A structural event is represented by the newly completed Ark bar/zone.
   // Consume it once so the same event cannot repeatedly compete with Trend.
   if(lastConsumedArkEvent>0 &&
      bestSignal.zoneTime<=lastConsumedArkEvent)
      return false;

   if(GoldArkEventCooldownMinutes>0 && lastConsumedArkEvent>0 &&
      (bestSignal.zoneTime-lastConsumedArkEvent) <
      GoldArkEventCooldownMinutes*60)
      return false;

   lastConsumedArkEvent=bestSignal.zoneTime;
   a=bestSignal;
   return true;
}
...
double GoldCandlePressure(string symbol,ENUM_TIMEFRAMES tf,int count,double minBodyATR)
{
   if(count<=0) return 0.0;
   MqlRates r[];
   ArraySetAsSeries(r,true);
   int need=count+2;
   int copied=CopyRates(symbol,tf,1,need,r); // closed candles only
   if(copied<count) return 0.0;

   double atr=0.0;
   for(int i=0;i<copied;i++) atr+=r[i].high-r[i].low;
   atr/=MathMax(1,copied);
   if(atr<=0.0) return 0.0;
   ...
   for(int i=0;i<count;i++)
   {
      ...
      if(bodyAbs<atr*minBodyATR) return 0.0;
      int s=(body>0.0?1:(body<0.0?-1:0));
      if(s==0) return 0.0;
      if(i==0) sign=s;
   ...
   if(same<count) return 0.0;

   double strength=Clamp(bodySum/(double)count,0.0,1.0);
   return sign*strength;
}


double GoldSyntheticM15FromM5(string symbol,int count,double minBodyATR)
{
   if(count<3) count=3;

   MqlRates r[];
   ArraySetAsSeries(r,true);
   int copied=CopyRates(symbol,PERIOD_M5,1,count,r); // only CLOSED M5 candles
   if(copied<count) return 0.0;

   // Require the candles to belong to one real 15-minute bucket.
   datetime firstBucket=(datetime)((long)r[count-1].time - ((long)r[count-1].time % 900));
   datetime lastBucket =(datetime)((long)r[0].time       - ((long)r[0].time       % 900));
   if(firstBucket!=lastBucket) return 0.0;

   double high=r[0].high, low=r[0].low;
   double open=r[count-1].open, close=r[0].close;
   double range=high-low;
   if(range<=0.0) return 0.0;

   double body=MathAbs(close-open);
   if(body < range*minBodyATR) return 0.0;

   int dir=(close>open ? 1 : (close<open ? -1 : 0));
   if(dir==0) return 0.0;

   // Require the component M5 candles to agree directionally.
   int agree=0;
   for(int i=0;i<count;i++)
   {
      if((r[i].close-r[i].open)*dir > 0.0) agree++;
   }
   if(agree<count) return 0.0;

   // Body/range quality: a strong synthetic M15 candle gets more weight.
   return dir*Clamp(body/range,0.0,1.0);
}

// ARK owns the decision while its event/trade lifecycle is active.
bool   GoldArkTradeLock = false;
ulong  GoldArkPositionTicket = 0;
datetime GoldLastArkEventTime = 0;

bool GoldHasOwnPosition()
{
   if(!PositionSelect(GoldSymbol)) return false;
   long magic=(long)PositionGetInteger(POSITION_MAGIC);
   return (magic==GoldMagic);
}

bool GoldArkPositionStillActive()
{
   if(!GoldHasOwnPosition()) return false;
   long magic=(long)PositionGetInteger(POSITION_MAGIC);
   return (magic==GoldArkMagic);
}

void GoldUpdateArkLock(bool arkEventActive)
{
   if(arkEventActive)
   {
      GoldArkTradeLock=true;
      GoldLastArkEventTime=TimeCurrent();
      return;
   }

   if(GoldArkTradeLock && !GoldArkPositionStillActive())
      GoldArkTradeLock=false;
}


bool GoldSyntheticM15Confirmation(int &direction,double &strength)
{
   direction=0;
   strength=0.0;
   ...
   MqlRates r[];
   ArraySetAsSeries(r,true);
   int copied=CopyRates(GoldSymbol,PERIOD_M5,1,4,r); // last 3 closed M5 candles
   if(copied<3) return false;

   // Require the three candles to form one completed 15-minute block.
   datetime block0=r[0].time-(r[0].time%900);
   for(int i=1;i<3;i++)
      if((r[i].time-(r[i].time%900))!=block0) return false;

   int s0=(r[0].close>r[0].open)?1:((r[0].close<r[0].open)?-1:0);
   int s1=(r[1].close>r[1].open)?1:((r[1].close<r[1].open)?-1:0);
   int s2=(r[2].close>r[2].open)?1:((r[2].close<r[2].open)?-1:0);
   if(s0==0 || s0!=s1 || s0!=s2) return false;

   double range=(r[0].high-r[0].low)+(r[1].high-r[1].low)+(r[2].high-r[2].low);
   double body=MathAbs(r[2].open-r[0].close);
   if(range<=0.0) return false;

   direction=s0;
   strength=Clamp(body/range,0.0,1.0);
   return (strength>=0.20);
}

bool GoldM5M15TrendConfirmed(MARKET_STATE &m,int &direction)
{
   direction=0;
   ...
   double m5Candle=GoldUseM5CandlePressure
      ? GoldCandlePressure(GoldSymbol,PERIOD_M5,
                            GoldM5ConsecutiveCandleCount,GoldM5MinBodyATR)
      : 0.0;

   // Three closed M5 candles are treated as the fast M15 confirmation.
   // This prevents a lagging indicator-based M15 score from blocking an
   // already completed 15-minute directional move.
   double syntheticM15=GoldSyntheticM15FromM5(GoldSymbol,3,GoldM5MinBodyATR);

   bool m5Up=(m5>=M5MinTrendScoreForTrade);
   bool m5Dn=(m5<=-M5MinTrendScoreForTrade);
   bool candleUp=(m5Candle>0.0);
   bool candleDn=(m5Candle<0.0);

   // M5 may establish the developing intraday direction.
   if(m5Up || candleUp) direction=1;
   else if(m5Dn || candleDn) direction=-1;
   else return false;

   // The completed 3xM5 block is the M15 confirmation.
   if(syntheticM15!=0.0)
   {
      int sm15Dir=(syntheticM15>0.0 ? 1 : -1);
      if(sm15Dir!=direction) return false;
   }
   else
   {
      // If the three M5 bars do not form one clean 15-minute block,
      // fall back to the actual M15 trend score rather than guessing.
      if(direction>0 && m15 < -GoldMinimumDirectionalScore*0.50) return false;
      if(direction<0 && m15 >  GoldMinimumDirectionalScore*0.50) return false;
   }

   // Actual M15 indicator score is now a secondary veto, not the primary
   // confirmation. Only a strong opposite M15 reading blocks the trade.
   if(direction>0 && m15 < -GoldMinimumDirectionalScore*0.75) return false;
   if(direction<0 && m15 >  GoldMinimumDirectionalScore*0.75) return false;

   // M30 remains survival/context, not an entry trigger.
   if(direction>0 && m30 < -GoldM30SurvivalThreshold) return false;
   if(direction<0 && m30 >  GoldM30SurvivalThreshold) return false;

   return true;
}

bool GoldCadenceReady()
{
   if(GoldTradeCadenceMinutes<=0) return true;
   if(GoldLastEntryTime<=0) return true;
   return (TimeCurrent()-GoldLastEntryTime)>=GoldTradeCadenceMinutes*60;
}
...
void ManageGoldAnchor()
{
   ...
   if(GoldM1LiquidityEnabled && PrintExecutionLog)
   {
      ...
      if(DetectGoldM1Liquidity(ls) &&
         ...
         Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | M1 LIQUIDITY | sweepUp=",ls.bearishSweep,
   ...
   // ================================================================
   // NON-COLLIDING DECISION ARCHITECTURE — 5.8
   //
   // 1) ARK M15 event gets first right of execution.
   //    If an ARK event forms, Trend PAUSES until the ARK position/order
   //    is complete. There is no Trend-vs-ARK conflict engine.
   //
   // 2) If no ARK event exists, Trend requires M5 + M15 confirmation.
   //
   // 3) M30 is a survival/context check. Strong opposition pauses new
   //    Trend entries and forces reevaluation; it does not reverse trades.
   //
   // 4) The 15-minute cadence is a TARGET/eligibility window, never a
   //    fabricated trade signal. The bot cannot guarantee a trade every
   //    15 minutes without compromising strategy integrity.
   // ================================================================

   GOLD_ARK_SIGNAL ark;
   bool arkReady=EvaluateGoldArkStyle(ark,g);

   // ARK has priority while its event is active.
   // No Trend signal is evaluated/executed during an ARK event.
   if(arkReady)
   {
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.2 | ARK M15 EVENT PRIORITY | dir=",ark.direction,
               " score=",DoubleToString(ark.score,2),
               " | TREND PAUSED");

      if(PlaceGoldArkPending(g,ark))
      {
         GoldLastEntryTime=TimeCurrent();
         return;
      }

      // If ARK setup failed broker/execution validation, do not immediately
      // fall through to Trend on the same event; that would create a collision.
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.2 | ARK EVENT PRESENT BUT NOT EXECUTED | TREND REMAINS PAUSED");
      return;
   }

   if(!GoldAllowTrendTrading || !GoldTrendOnlyMode)
      return;

   // 15-minute cadence is an eligibility target, not a forced trade.
   if(!GoldCadenceReady())
   {
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.2 | CADENCE WAIT | next Trend window in ",
               IntegerToString(MathMax(0,(int)(GoldTradeCadenceMinutes*60-(TimeCurrent()-GoldLastEntryTime)))),
               " sec");
      return;
   }

   int trendDirection=0;
   bool trendConfirmed=GoldM5M15TrendConfirmed(g,trendDirection);

   double m5=MathAbs(g.tf[1].trend);
   double m15=MathAbs(g.tf[2].trend);
   double m30=g.tf[3].trend;

   // M30 survival check is explicit in the log.
   bool m30Survives=true;
   if(trendDirection>0 && m30 < -GoldM30SurvivalThreshold) m30Survives=false;
   if(trendDirection<0 && m30 >  GoldM30SurvivalThreshold) m30Survives=false;

   double m5CandlePressure=GoldUseM5CandlePressure
      ? MathAbs(GoldCandlePressure(GoldSymbol,PERIOD_M5,GoldM5ConsecutiveCandleCount,GoldM5MinBodyATR))
      : 0.0;
   double syntheticM15Pressure=MathAbs(
      GoldSyntheticM15FromM5(GoldSymbol,3,GoldM5MinBodyATR));
   double m15CandlePressure=GoldUseM15CandlePressure
      ? MathAbs(GoldCandlePressure(GoldSymbol,PERIOD_M15,MathMax(2,GoldM5ConsecutiveCandleCount-1),GoldM5MinBodyATR))
      : 0.0;

   double normalScore=
      Clamp(g.opportunityScore/100.0,0.0,1.0)*0.25+
      m5*0.25+
      m15*0.20+
      m5CandlePressure*GoldM5CandlePressureWeight+
      m15CandlePressure*0.10+
      Clamp(MathAbs(m30),0.0,1.0)*0.10;

   if(GoldRequireM5M15Confirmation && !trendConfirmed)
   {
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.2 | TREND WAIT | M5=",DoubleToString(g.tf[1].trend,2),
               " M15=",DoubleToString(g.tf[2].trend,2),
               " M30=",DoubleToString(m30,2),
               " | M5+M15 NOT CONFIRMED");
      return;
   }
   ...
   {
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.2 | M30 SURVIVAL FAIL | Trend=",trendDirection,
   ...
   if(PrintExecutionLog)
      Print("NeoFL GOLD 6.2 | TREND CONFIRMED | M5=",DoubleToString(g.tf[1].trend,2),
            " M15=",DoubleToString(g.tf[2].trend,2),
            " M30=",DoubleToString(m30,2),
            " M5Candle=",DoubleToString(m5CandlePressure,2),
            " SynthM15=",DoubleToString(syntheticM15Pressure,2)," M15Candle=",DoubleToString(m15CandlePressure,2),
   ...
   if(PrintExecutionLog)
      Print("NeoFL GOLD 6.2 | TREND SIGNAL QUALIFIED BUT ORDER NOT ACCEPTED");
...
bool PlaceTrendPending(MARKET_STATE &m)
{
   ...
   {
      if(PrintExecutionLog) Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | ACCOUNT MARGIN BLOCK | ",symbol," | dedicated-account margin protection");
      return false;
   }
   bool ok=false;
   if(PrintExecutionLog) Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | TREND THESIS | ",symbol," dir=",m.direction," score=",DoubleToString(m.opportunityScore,1)," entry=",DoubleToString(entry,DigitsOf(symbol))," tp=",DoubleToString(tp,DigitsOf(symbol)));
   if(!DryRun)
   {
             trade.SetExpertMagicNumber(BotMagicNumber);
if(m.direction>0) ok=trade.BuyStop(lots,entry,symbol,sl,tp,ORDER_TIME_GTC,0,"NeoFL4.2 TREND BUY");
      ...
      if(!ok && PrintExecutionLog) Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | ORDER FAILED | retcode=",trade.ResultRetcode()," ",trade.ResultRetcodeDescription());
   }
   else ok=true;
   if(ok)
   {
...
bool PlaceSidewaysStraddle(MARKET_STATE &m)
{
   ...
   double lots=CalculateLots(symbol,slDist/point);
    ...
    {
       if(PrintRiskManagerLog)
          Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING CAPITAL | ACCOUNT MARGIN BLOCK | ",symbol,
                " | dedicated-account margin protection");
       return false;
    }

    bool ok1=true,ok2=true;
   if(PrintExecutionLog) Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | SIDEWAYS STRADDLE | ",symbol," buy=",DoubleToString(buy,DigitsOf(symbol))," sell=",DoubleToString(sell,DigitsOf(symbol)));
   if(!DryRun)
   {
             trade.SetExpertMagicNumber(BotMagicNumber);
ok1=trade.BuyStop(lots,buy,symbol,buySL,buyTP,ORDER_TIME_GTC,0,"NeoFL4.2 RANGE BUY");
      if(!ok1 && PrintExecutionLog) Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | BUY STOP FAILED | ",trade.ResultRetcodeDescription());
      ok2=trade.SellStop(lots,sell,symbol,sellSL,sellTP,ORDER_TIME_GTC,0,"NeoFL4.2 RANGE SELL");
      if(!ok2 && PrintExecutionLog) Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | SELL STOP FAILED | ",trade.ResultRetcodeDescription());
   }
   if(ok1||ok2)
   {
...
bool ManageCandleStructureTrail()
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      {
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | CANDLE TRAIL FAILED | ",symbol,
                  " ticket=",ticket,
      ...
      else
      {
         any=true;
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | CANDLE TRAIL | ",symbol,
                  " ticket=",ticket,
...
bool ManageAllPositions()
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      {
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | ",reason," FAILED | ",symbol,
                  " ticket=",ticket,
                  " retcode=",trade.ResultRetcode(),
                  " ",trade.ResultRetcodeDescription(),
                  " oldSL=",DoubleToString(sl,DigitsOf(symbol)),
      ...
      {
         Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | ",reason," | ",symbol,
               " ticket=",ticket,
               " profitR=",DoubleToString(profitR,2),
...
void EvaluatePendingThesis()
{
   ...
   {
      if(PrintExecutionLog) Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | THESIS EXPIRED | ",PendingSymbol);
   ...
   {
      if(PrintExecutionLog) Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | THESIS INVALIDATED | ",PendingSymbol," trend=",DoubleToString(m.trend,2));
...
void FindAndExecuteBest()
{
   ...
   if(m.regime==MARKET_STRONG_UP || m.regime==MARKET_STRONG_DOWN)
   {
      ...
      {
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | M5 PRIMARY TREND GATE BLOCK | symbol=",m.symbol,
...
bool GoldTrendPausedByArk()
{
   if(!GoldArkPausesTrend) return false;
   return GoldArkTradeLock;
}
...
int OnInit()
{
   ...
   Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | DEDICATED ACCOUNT | "
         "STRATEGY CAPITAL=100% | NO STANDALONE MARKET | BASE BALANCE=",
         DoubleToString(CapitalLedgerStartBalance,2));

   if(MarketCount==0)
   {
      Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | NO USABLE "
   ...
   Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING STARTED | "
         "DEDICATED ACTIVE SCANNER | symbols=",MarketCount,
         " | scan=",ScanSeconds,"s | host=",_Symbol);

   Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | Scanner reads only the "
                        ...
                        const MqlTradeResult &result)
{
      ...
      GoldBlockOppositeAfterStop)
   {
         ...
         (GoldShockEventBar>0 && TimeCurrent()-GoldShockEventBar<=GoldShockLockSeconds))
      {
         ...
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | STOP DURING/AT LIQUIDITY EVENT | ",
   ...
   if(PrintExecutionLog)
      Print("NeoFL GOLD 6.2 TREND + ARK NON-COLLIDING | FILL DETECTED | ",symbol,
...
void OnTick()
{
   // 6.2 architecture status: ARK and Trend scan independently.
   // Normal Trend entries are blocked only while ARK owns an active lifecycle.
   static datetime lastStatus=0;
   if(TimeCurrent()-lastStatus>=60)
   {
      Print("NeoFL GOLD 6.2 | ARCHITECTURE | TREND=M5+SYNTH_M15 | M30=SURVIVAL | ",
            "M1=LIQUIDITY/TRAIL | SMC=",GoldUseSMCConfluence,
            " SD=",GoldUseSupplyDemandConfluence,
            " ARK_LOCK=",GoldArkTradeLock);
