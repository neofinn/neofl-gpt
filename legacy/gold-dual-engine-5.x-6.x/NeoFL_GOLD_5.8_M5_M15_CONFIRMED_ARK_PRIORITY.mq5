...
bool GoldM5M15TrendConfirmed(MARKET_STATE &m,int &direction)
{
   ...
   if(MathAbs(m5)<M5MinTrendScoreForTrade) return false;
   if(MathAbs(m15)<GoldMinimumDirectionalScore*0.75) return false;

   if(m5>0.0 && m15>0.0)
      direction=1;
   else if(m5<0.0 && m15<0.0)
      direction=-1;
   else
      return false;

   // M30 does not need to "agree" tick-for-tick. It is a survival filter.
   // Strong opposition means the current trend needs reevaluation.
...
void ManageGoldAnchor()
{
   ...
   double normalScore=
      Clamp(g.opportunityScore/100.0,0.0,1.0)*0.35+
      m5*0.35+
      m15*0.20+
