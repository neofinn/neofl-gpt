...
//|                                                                  |
//| NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING - Dedicated Gold Active Scanner                     |
//|                                                                  |
//| PURPOSE                                                          |
...
// ================================================================
// ================================================================
// NeoFL GOLD 6.3 — UNIFIED DUAL-ENGINE ARCHITECTURE
...
#property description "NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING - Standalone GOLD Trading Bot"
...
// NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING: GOLD uses the dedicated MT5 account as its full strategy pool. No cross-bot capital sharing.
   ...
   int aliasCount)
{
   ...
   {
      Print(
         "NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | AUTO MAP | ",
         canonical,
         " -> NOT FOUND"
      );
      return;
...
void BuildAutoDetectedUniverse()
{
   ...
   Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | DEDICATED ACTIVE SCAN | GOLD symbols=",MarketCount);
   if(GoldSymbol!="")
      Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | ACTIVE SYMBOL=",GoldSymbol);
   else
      Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | GOLD/XAU SYMBOL NOT FOUND");
}
...
void BuildUniverse()
{
   ...
   for(int i=0;i<n;i++)
   {
      ...
      if(requested=="") continue;       if(!IsGoldSymbolName(requested))
       {
          Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | MANUAL SYMBOL REJECTED (NOT GOLD): ",requested);
         continue;
      }
   ...
   Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | MANUAL GOLD UNIVERSE=",MarketCount);
}
...
void ScanMarkets()
{
   ...
   for(int i=0;i<MarketCount;i++)
   {
      ...
      {
         if(PrintSymbolMapping)
            Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | SCAN REJECTED NON-GOLD: ",Symbols[i]);
         continue;
      }
...
void PrintTopMarkets()
{
   ...
   Print(
      "================ NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING ACTIVE OPPORTUNITY SCAN ================"
   );
   ...
   {
      Print(
         "NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | BEST OPPORTUNITY = ",
   ...
   {
      Print(
         "NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | NO QUALIFIED OPPORTUNITY"
      );
   }
...
void DisplayDashboard()
{
   ...
   string text=
      "NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING - STANDALONE ACTIVE SCANNER\n";
   text+=
      "GLOBAL SESSION: "+
      currentSession+
      "\n";
...
void CancelOurPending(string symbol="")
{
   for(int i=OrdersTotal()-1;i>=0;i--)
   {
      ...
      if(ot==ORDER_TYPE_BUY_STOP || ot==ORDER_TYPE_SELL_STOP)
      {
         ...
         if(PrintExecutionLog) Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | CANCEL PENDING | ",os," ticket=",ticket);
      }
   }
                           ...
                           double entry)
{
   ...
   if(PrintRiskManagerLog)
      Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING CAPITAL | DEDICATED ACCOUNT",
                                  ...
                                  double entry2)
{
   ...
   if(PrintRiskManagerLog)
      Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING CAPITAL | DEDICATED COMBINED",
            " equity=",DoubleToString(accountEquity,2),
            " usable=",DoubleToString(usable,2),
            " required=",DoubleToString(required,2),
            " free=",DoubleToString(accountFree,2),
...
void CancelPendingAfterFill(string symbol)
{
   ...
   if(symbol==GoldSymbol && GoldNewsActive && GoldNewsStraddleArmed)
   {
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | NEWS RIDER | opposite stop retained");
      return;
   }
         ...
         ot==ORDER_TYPE_BUY_LIMIT || ot==ORDER_TYPE_SELL_LIMIT)
      {
         if(!DryRun)
         {
            if(!trade.OrderDelete(ticket) && PrintExecutionLog)
               Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | OCO DELETE FAILED | ",symbol,
                     " ticket=",ticket," retcode=",trade.ResultRetcode(),
                     " ",trade.ResultRetcodeDescription());
         }
         else if(PrintExecutionLog)
            Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | DRY OCO DELETE | ",symbol," ticket=",ticket);
      }
   }
...
void ProtectGoldPositionOnLiquidityShock(int shockDirection,double atrPrice)
{
   ...
   if(pt==POSITION_TYPE_BUY && shockDirection<0 && bid>0)
   {
      ...
      if(oldSL<=0 || newSL>oldSL)
      {
         ...
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | SHOCK PROTECT BUY | SL ",
   ...
   if(pt==POSITION_TYPE_SELL && shockDirection>0 && ask>0)
   {
      ...
      if(oldSL<=0 || newSL<oldSL)
      {
         if(!DryRun) trade.PositionModify(GoldSymbol,newSL,tp);
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | SHOCK PROTECT SELL | SL ",
                  DoubleToString(oldSL,DigitsOf(GoldSymbol))," -> ",
                  DoubleToString(newSL,DigitsOf(GoldSymbol)));
      }
   }
...
bool PlaceGoldNewsStraddle()
{
   ...
   if(a && b)
   {
      ...
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | NEWS STRADDLE ARMED | ",GoldNewsEventName,
...
void TrailGoldNewsRider()
{
   ...
   else if(dir!=GoldNewsPositionDirection)
   {
      ...
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | NEWS RIDER REVERSAL | direction=",dir,
   ...
   for(int i=OrdersTotal()-1;i>=0;i--)
   {
      ...
      if(PrintExecutionLog && moved)
         Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | NEWS RIDER TRAIL | posSL=",
               DoubleToString(newSL,DigitsOf(GoldSymbol)),
...
void UpdateGoldNewsEngine()
{
   ...
   if(GoldNewsStraddleArmed)
   {
      ...
      if(!PositionSelect(GoldSymbol))
      {
         ...
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | NEWS CAMPAIGN COMPLETE | recovery lock started");
      }
   }
}
...
void UpdateGoldLiquidityDefense()
{
   ...
   if(GoldShockActive)
   {
      if(TimeCurrent()>=GoldShockUntil && GoldShockConfirmationComplete())
      {
         ...
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | SHOCK LOCK CLEARED | scanning resumed");
      }
      return;
   }
   ...
   if(PrintExecutionLog)
      Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | LIQUIDITY SWEEP | ",
...
bool PlaceGoldTrendPending(MARKET_STATE &m)
{
   ...
   {
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | SMC REQUIRED BLOCK | dir=",tradeDirection,
   ...
   {
      if(PrintRiskManagerLog)
         Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING CAPITAL | GOLD ORDER BLOCKED | bucket limit");
      return false;
   }

   bool ok=false;
   if(PrintExecutionLog)
      Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | SMC TREND | dir=",tradeDirection,
   ...
   if(!DryRun)
   {
      ...
      if(!ok && PrintExecutionLog)
         Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | ORDER FAILED | retcode=",
...
bool PlaceGoldSidewaysStraddle(MARKET_STATE &m)
{
   ...
   double lots=CalculateLots(GoldSymbol,slDist/point);    if(!AccountCapitalAllowsCombined(GoldSymbol,ORDER_TYPE_BUY_STOP,lots,buy,GoldSymbol,ORDER_TYPE_SELL_STOP,lots,sell))
    {
       if(PrintRiskManagerLog)
          Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING CAPITAL | GOLD RANGE BLOCKED | bucket limit");
       return false;
    }

    bool ok1=true,ok2=true;
   if(PrintExecutionLog)
      Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | GOLD SIDEWAYS STRADDLE | ",GoldSymbol," buy=",DoubleToString(buy,DigitsOf(GoldSymbol))," sell=",DoubleToString(sell,DigitsOf(GoldSymbol)));
   if(!DryRun)
   {
             trade.SetExpertMagicNumber(BotMagicNumber);
ok1=trade.BuyStop(lots,buy,GoldSymbol,buySL,buyTP,ORDER_TIME_GTC,0,"NeoFL4.2 GOLD RANGE BUY");
      if(!ok1 && PrintExecutionLog) Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | GOLD BUY STOP FAILED | ",trade.ResultRetcode()," ",trade.ResultRetcodeDescription());
      ok2=trade.SellStop(lots,sell,GoldSymbol,sellSL,sellTP,ORDER_TIME_GTC,0,"NeoFL4.2 GOLD RANGE SELL");
      if(!ok2 && PrintExecutionLog) Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | GOLD SELL STOP FAILED | ",trade.ResultRetcode()," ",trade.ResultRetcodeDescription());
   }
   if(ok1||ok2)
   {
      GoldPendingSymbol=GoldSymbol;
...
void ManageGoldAnchor()
{
   ...
   if(GoldM1LiquidityEnabled && PrintExecutionLog)
   {
      ...
      if(DetectGoldM1Liquidity(ls) &&
         ...
         Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | M1 LIQUIDITY | sweepUp=",ls.bearishSweep,
   ...
   // ================================================================
   // UNIFIED DUAL-ENGINE DECISION ARCHITECTURE — 6.3
   //
   ...
   {
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.3 | ARK M15 EVENT PRIORITY | dir=",ark.direction,
      ...
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.3 | ARK EVENT PRESENT BUT NOT EXECUTED | TREND REMAINS PAUSED");
      return;
   }
   ...
   {
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.3 | CADENCE WAIT | next Trend window in ",
   ...
   {
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.3 | TREND WAIT | M5=",DoubleToString(g.tf[1].trend,2),
   ...
   {
      if(PrintExecutionLog)
         Print("NeoFL GOLD 6.3 | M30 SURVIVAL FAIL | Trend=",trendDirection,
   ...
   if(PrintExecutionLog)
      Print("NeoFL GOLD 6.3 | TREND CONFIRMED | M5=",DoubleToString(g.tf[1].trend,2),
            " M15=",DoubleToString(g.tf[2].trend,2),
            " M30=",DoubleToString(m30,2),
   ...
   if(PrintExecutionLog)
      Print("NeoFL GOLD 6.3 | TREND SIGNAL QUALIFIED BUT ORDER NOT ACCEPTED");
...
bool PlaceTrendPending(MARKET_STATE &m)
{
   ...
   {
      if(PrintExecutionLog) Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | ACCOUNT MARGIN BLOCK | ",symbol," | dedicated-account margin protection");
      return false;
   }
   bool ok=false;
   if(PrintExecutionLog) Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | TREND THESIS | ",symbol," dir=",m.direction," score=",DoubleToString(m.opportunityScore,1)," entry=",DoubleToString(entry,DigitsOf(symbol))," tp=",DoubleToString(tp,DigitsOf(symbol)));
   if(!DryRun)
   {
             trade.SetExpertMagicNumber(BotMagicNumber);
if(m.direction>0) ok=trade.BuyStop(lots,entry,symbol,sl,tp,ORDER_TIME_GTC,0,"NeoFL4.2 TREND BUY");
      ...
      if(!ok && PrintExecutionLog) Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | ORDER FAILED | retcode=",trade.ResultRetcode()," ",trade.ResultRetcodeDescription());
   }
   else ok=true;
   if(ok)
   {
...
bool PlaceSidewaysStraddle(MARKET_STATE &m)
{
   ...
   double lots=CalculateLots(symbol,slDist/point);
    ...
    {
       if(PrintRiskManagerLog)
          Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING CAPITAL | ACCOUNT MARGIN BLOCK | ",symbol,
                " | dedicated-account margin protection");
       return false;
    }

    bool ok1=true,ok2=true;
   if(PrintExecutionLog) Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | SIDEWAYS STRADDLE | ",symbol," buy=",DoubleToString(buy,DigitsOf(symbol))," sell=",DoubleToString(sell,DigitsOf(symbol)));
   if(!DryRun)
   {
             trade.SetExpertMagicNumber(BotMagicNumber);
ok1=trade.BuyStop(lots,buy,symbol,buySL,buyTP,ORDER_TIME_GTC,0,"NeoFL4.2 RANGE BUY");
      if(!ok1 && PrintExecutionLog) Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | BUY STOP FAILED | ",trade.ResultRetcodeDescription());
      ok2=trade.SellStop(lots,sell,symbol,sellSL,sellTP,ORDER_TIME_GTC,0,"NeoFL4.2 RANGE SELL");
      if(!ok2 && PrintExecutionLog) Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | SELL STOP FAILED | ",trade.ResultRetcodeDescription());
   }
   if(ok1||ok2)
   {
...
bool ManageCandleStructureTrail()
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      {
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | CANDLE TRAIL FAILED | ",symbol,
                  " ticket=",ticket,
      ...
      else
      {
         any=true;
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | CANDLE TRAIL | ",symbol,
                  " ticket=",ticket,
...
bool ManageAllPositions()
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      {
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | ",reason," FAILED | ",symbol,
                  " ticket=",ticket,
                  " retcode=",trade.ResultRetcode(),
                  " ",trade.ResultRetcodeDescription(),
                  " oldSL=",DoubleToString(sl,DigitsOf(symbol)),
      ...
      {
         Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | ",reason," | ",symbol,
               " ticket=",ticket,
               " profitR=",DoubleToString(profitR,2),
...
void EvaluatePendingThesis()
{
   ...
   {
      if(PrintExecutionLog) Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | THESIS EXPIRED | ",PendingSymbol);
   ...
   {
      if(PrintExecutionLog) Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | THESIS INVALIDATED | ",PendingSymbol," trend=",DoubleToString(m.trend,2));
...
void FindAndExecuteBest()
{
   ...
   if(m.regime==MARKET_STRONG_UP || m.regime==MARKET_STRONG_DOWN)
   {
      ...
      {
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | M5 PRIMARY TREND GATE BLOCK | symbol=",m.symbol,
...
int OnInit()
{
   ...
   Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | DEDICATED ACCOUNT | "
         "STRATEGY CAPITAL=100% | NO STANDALONE MARKET | BASE BALANCE=",
         DoubleToString(CapitalLedgerStartBalance,2));

   if(MarketCount==0)
   {
      Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | NO USABLE "
   ...
   Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING STARTED | "
         "DEDICATED ACTIVE SCANNER | symbols=",MarketCount,
         " | scan=",ScanSeconds,"s | host=",_Symbol);

   Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | Scanner reads only the "
                        ...
                        const MqlTradeResult &result)
{
      ...
      GoldBlockOppositeAfterStop)
   {
         ...
         (GoldShockEventBar>0 && TimeCurrent()-GoldShockEventBar<=GoldShockLockSeconds))
      {
         ...
         if(PrintExecutionLog)
            Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | STOP DURING/AT LIQUIDITY EVENT | ",
   ...
   if(PrintExecutionLog)
      Print("NeoFL GOLD 6.3 TREND + ARK NON-COLLIDING | FILL DETECTED | ",symbol,
...
void OnTick()
{
   ...
   {
      Print("NeoFL GOLD 6.3 | ARCHITECTURE | TREND=M5+SYNTH_M15 | M30=SURVIVAL | ",
