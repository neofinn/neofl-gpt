...
//|                                                                  |
//| NeoFL GOLD 5.2 FIX1 - Dedicated Gold Active Scanner                     |
//|                                                                  |
//| PURPOSE                                                          |
...
#property description "NeoFL GOLD 5.2 FIX1 - Standalone GOLD Trading Bot"
...
// NeoFL GOLD 5.2 FIX1: GOLD uses the dedicated MT5 account as its full strategy pool. No cross-bot capital sharing.
...
input double GoldM5TrendWeight = 0.45;
                          ...
                          MqlRates &htf[],bool m1Liquidity)
{
   ...
   double score=alignedM5*GoldM5TrendWeight+
...
void BuildAutoDetectedUniverse()
{
   ...
   Print("NeoFL GOLD 5.2 FIX1 | DEDICATED ACTIVE SCAN | GOLD symbols=",MarketCount);
   if(GoldSymbol!="")
      Print("NeoFL GOLD 5.2 FIX1 | ACTIVE SYMBOL=",GoldSymbol);
   else
      Print("NeoFL GOLD 5.2 FIX1 | GOLD/XAU SYMBOL NOT FOUND");
}
...
void BuildUniverse()
{
   ...
   for(int i=0;i<n;i++)
   {
      ...
      if(StringFind(u,"XAU")<0 && StringFind(u,"GOLD")<0)
      {
         Print("NeoFL GOLD 5.2 FIX1 | MANUAL SYMBOL REJECTED (NOT GOLD): ",requested);
         continue;
      }
   ...
   Print("NeoFL GOLD 5.2 FIX1 | MANUAL GOLD UNIVERSE=",MarketCount);
}
...
void PrintTopMarkets()
{
   ...
   Print(
      "================ NeoFL GOLD 5.2 FIX1 ACTIVE OPPORTUNITY SCAN ================"
   );
   ...
   {
      Print(
         "NeoFL GOLD 5.2 FIX1 | BEST OPPORTUNITY = ",
   ...
   {
      Print(
         "NeoFL GOLD 5.2 FIX1 | NO QUALIFIED OPPORTUNITY"
      );
   }
...
void DisplayDashboard()
{
   ...
   string text=
      "NeoFL GOLD 5.2 FIX1 - STANDALONE ACTIVE SCANNER\n";
   text+=
      "GLOBAL SESSION: "+
      currentSession+
      "\n";
...
void CancelOurPending(string symbol="")
{
   for(int i=OrdersTotal()-1;i>=0;i--)
   {
      ...
      if(ot==ORDER_TYPE_BUY_STOP || ot==ORDER_TYPE_SELL_STOP)
      {
         ...
         if(PrintExecutionLog) Print("NeoFL GOLD 5.2 FIX1 | CANCEL PENDING | ",os," ticket=",ticket);
      }
   }
                           ...
                           double entry)
{
   ...
   if(PrintRiskManagerLog)
      Print("NeoFL GOLD 5.2 FIX1 CAPITAL | DEDICATED ACCOUNT",
                                  ...
                                  double entry2)
{
   ...
   if(PrintRiskManagerLog)
      Print("NeoFL GOLD 5.2 FIX1 CAPITAL | DEDICATED COMBINED",
            " equity=",DoubleToString(accountEquity,2),
            " usable=",DoubleToString(usable,2),
            " required=",DoubleToString(required,2),
            " free=",DoubleToString(accountFree,2),
...
void CancelPendingAfterFill(string symbol)
{
   ...
   if(symbol==GoldSymbol && GoldNewsActive && GoldNewsStraddleArmed)
   {
      if(PrintExecutionLog)
         Print("NeoFL GOLD 5.2 FIX1 | NEWS RIDER | opposite stop retained");
      return;
   }
         ...
         ot==ORDER_TYPE_BUY_LIMIT || ot==ORDER_TYPE_SELL_LIMIT)
      {
         if(!DryRun)
         {
            if(!trade.OrderDelete(ticket) && PrintExecutionLog)
               Print("NeoFL GOLD 5.2 FIX1 | OCO DELETE FAILED | ",symbol,
                     " ticket=",ticket," retcode=",trade.ResultRetcode(),
                     " ",trade.ResultRetcodeDescription());
         }
         else if(PrintExecutionLog)
            Print("NeoFL GOLD 5.2 FIX1 | DRY OCO DELETE | ",symbol," ticket=",ticket);
      }
   }
...
void ProtectGoldPositionOnLiquidityShock(int shockDirection,double atrPrice)
{
   ...
   if(pt==POSITION_TYPE_BUY && shockDirection<0 && bid>0)
   {
      ...
      if(oldSL<=0 || newSL>oldSL)
      {
         ...
         if(PrintExecutionLog)
            Print("NeoFL GOLD 5.2 FIX1 | SHOCK PROTECT BUY | SL ",
   ...
   if(pt==POSITION_TYPE_SELL && shockDirection>0 && ask>0)
   {
      ...
      if(oldSL<=0 || newSL<oldSL)
      {
         if(!DryRun) trade.PositionModify(GoldSymbol,newSL,tp);
         if(PrintExecutionLog)
            Print("NeoFL GOLD 5.2 FIX1 | SHOCK PROTECT SELL | SL ",
                  DoubleToString(oldSL,DigitsOf(GoldSymbol))," -> ",
                  DoubleToString(newSL,DigitsOf(GoldSymbol)));
      }
   }
...
bool PlaceGoldNewsStraddle()
{
   ...
   if(a && b)
   {
      ...
      if(PrintExecutionLog)
         Print("NeoFL GOLD 5.2 FIX1 | NEWS STRADDLE ARMED | ",GoldNewsEventName,
...
void TrailGoldNewsRider()
{
   ...
   else if(dir!=GoldNewsPositionDirection)
   {
      ...
      if(PrintExecutionLog)
         Print("NeoFL GOLD 5.2 FIX1 | NEWS RIDER REVERSAL | direction=",dir,
   ...
   for(int i=OrdersTotal()-1;i>=0;i--)
   {
      ...
      if(PrintExecutionLog && moved)
         Print("NeoFL GOLD 5.2 FIX1 | NEWS RIDER TRAIL | posSL=",
               DoubleToString(newSL,DigitsOf(GoldSymbol)),
...
void UpdateGoldNewsEngine()
{
   ...
   if(GoldNewsStraddleArmed)
   {
      ...
      if(!PositionSelect(GoldSymbol))
      {
         ...
         if(PrintExecutionLog)
            Print("NeoFL GOLD 5.2 FIX1 | NEWS CAMPAIGN COMPLETE | recovery lock started");
      }
   }
}
...
void UpdateGoldLiquidityDefense()
{
   ...
   if(GoldShockActive)
   {
      if(TimeCurrent()>=GoldShockUntil && GoldShockConfirmationComplete())
      {
         ...
         if(PrintExecutionLog)
            Print("NeoFL GOLD 5.2 FIX1 | SHOCK LOCK CLEARED | scanning resumed");
      }
      return;
   }
   ...
   if(PrintExecutionLog)
      Print("NeoFL GOLD 5.2 FIX1 | LIQUIDITY SWEEP | ",
...
bool PlaceGoldTrendPending(MARKET_STATE &m)
{
   ...
   {
      if(PrintExecutionLog)
         Print("NeoFL GOLD 5.2 FIX1 | SMC REQUIRED BLOCK | dir=",tradeDirection,
   ...
   {
      if(PrintRiskManagerLog)
         Print("NeoFL GOLD 5.2 FIX1 CAPITAL | GOLD ORDER BLOCKED | bucket limit");
      return false;
   }

   bool ok=false;
   if(PrintExecutionLog)
      Print("NeoFL GOLD 5.2 FIX1 | SMC TREND | dir=",tradeDirection,
   ...
   if(!DryRun)
   {
      ...
      if(!ok && PrintExecutionLog)
         Print("NeoFL GOLD 5.2 FIX1 | ORDER FAILED | retcode=",
...
bool PlaceGoldSidewaysStraddle(MARKET_STATE &m)
{
   ...
   double lots=CalculateLots(GoldSymbol,slDist/point);    if(!AccountCapitalAllowsCombined(GoldSymbol,ORDER_TYPE_BUY_STOP,lots,buy,GoldSymbol,ORDER_TYPE_SELL_STOP,lots,sell))
    {
       if(PrintRiskManagerLog)
          Print("NeoFL GOLD 5.2 FIX1 CAPITAL | GOLD RANGE BLOCKED | bucket limit");
       return false;
    }

    bool ok1=true,ok2=true;
   if(PrintExecutionLog)
      Print("NeoFL GOLD 5.2 FIX1 | GOLD SIDEWAYS STRADDLE | ",GoldSymbol," buy=",DoubleToString(buy,DigitsOf(GoldSymbol))," sell=",DoubleToString(sell,DigitsOf(GoldSymbol)));
   if(!DryRun)
   {
             trade.SetExpertMagicNumber(BotMagicNumber);
ok1=trade.BuyStop(lots,buy,GoldSymbol,buySL,buyTP,ORDER_TIME_GTC,0,"NeoFL4.2 GOLD RANGE BUY");
      if(!ok1 && PrintExecutionLog) Print("NeoFL GOLD 5.2 FIX1 | GOLD BUY STOP FAILED | ",trade.ResultRetcode()," ",trade.ResultRetcodeDescription());
      ok2=trade.SellStop(lots,sell,GoldSymbol,sellSL,sellTP,ORDER_TIME_GTC,0,"NeoFL4.2 GOLD RANGE SELL");
      if(!ok2 && PrintExecutionLog) Print("NeoFL GOLD 5.2 FIX1 | GOLD SELL STOP FAILED | ",trade.ResultRetcode()," ",trade.ResultRetcodeDescription());
   }
   if(ok1||ok2)
   {
      GoldPendingSymbol=GoldSymbol;
...
void ManageGoldAnchor()
{
   ...
   if(GoldM1LiquidityEnabled && PrintExecutionLog)
   {
      ...
      if(DetectGoldM1Liquidity(ls) &&
         ...
         Print("NeoFL GOLD 5.2 FIX1 | M1 LIQUIDITY | sweepUp=",ls.bearishSweep,
   ...
   if(GoldTrendOnlyMode &&
      ...
      (g.regime==MARKET_STRONG_UP || g.regime==MARKET_STRONG_DOWN))
   {
      normalDirection=g.direction;
      normalReady=M5TrendGatePasses(g);
      ...
      if(PrintExecutionLog)
         Print("NeoFL GOLD 5.2 FIX1 | NORMAL ENGINE | ready=",normalReady,
               " dir=",normalDirection,
   ...
   if(PrintExecutionLog)
      Print("NeoFL GOLD 5.2 FIX1 | ARK ENGINE | ready=",arkReady,
            " dir=",ark.direction,
   ...
   if(arkReady && (!normalReady || ark.score>=normalScore))
   {
      ...
      {
         if(PrintExecutionLog)
            Print("NeoFL GOLD 5.2 FIX1 | ENGINE WINNER = ARK | score=",
   ...
   if(normalReady)
   {
      ...
      {
         if(PrintExecutionLog)
            Print("NeoFL GOLD 5.2 FIX1 | ENGINE WINNER = NORMAL TREND | score=",
   ...
   if(PrintExecutionLog)
      Print("NeoFL GOLD 5.2 FIX1 | BOTH ENGINES WAIT | M5=",
...
bool PlaceTrendPending(MARKET_STATE &m)
{
   ...
   {
      if(PrintExecutionLog) Print("NeoFL GOLD 5.2 FIX1 | ACCOUNT MARGIN BLOCK | ",symbol," | dedicated-account margin protection");
      return false;
   }
   bool ok=false;
   if(PrintExecutionLog) Print("NeoFL GOLD 5.2 FIX1 | TREND THESIS | ",symbol," dir=",m.direction," score=",DoubleToString(m.opportunityScore,1)," entry=",DoubleToString(entry,DigitsOf(symbol))," tp=",DoubleToString(tp,DigitsOf(symbol)));
   if(!DryRun)
   {
             trade.SetExpertMagicNumber(BotMagicNumber);
if(m.direction>0) ok=trade.BuyStop(lots,entry,symbol,sl,tp,ORDER_TIME_GTC,0,"NeoFL4.2 TREND BUY");
      ...
      if(!ok && PrintExecutionLog) Print("NeoFL GOLD 5.2 FIX1 | ORDER FAILED | retcode=",trade.ResultRetcode()," ",trade.ResultRetcodeDescription());
   }
   else ok=true;
   if(ok)
   {
...
bool PlaceSidewaysStraddle(MARKET_STATE &m)
{
   ...
   double lots=CalculateLots(symbol,slDist/point);
    ...
    {
       if(PrintRiskManagerLog)
          Print("NeoFL GOLD 5.2 FIX1 CAPITAL | ACCOUNT MARGIN BLOCK | ",symbol,
                " | dedicated-account margin protection");
       return false;
    }

    bool ok1=true,ok2=true;
   if(PrintExecutionLog) Print("NeoFL GOLD 5.2 FIX1 | SIDEWAYS STRADDLE | ",symbol," buy=",DoubleToString(buy,DigitsOf(symbol))," sell=",DoubleToString(sell,DigitsOf(symbol)));
   if(!DryRun)
   {
             trade.SetExpertMagicNumber(BotMagicNumber);
ok1=trade.BuyStop(lots,buy,symbol,buySL,buyTP,ORDER_TIME_GTC,0,"NeoFL4.2 RANGE BUY");
      if(!ok1 && PrintExecutionLog) Print("NeoFL GOLD 5.2 FIX1 | BUY STOP FAILED | ",trade.ResultRetcodeDescription());
      ok2=trade.SellStop(lots,sell,symbol,sellSL,sellTP,ORDER_TIME_GTC,0,"NeoFL4.2 RANGE SELL");
      if(!ok2 && PrintExecutionLog) Print("NeoFL GOLD 5.2 FIX1 | SELL STOP FAILED | ",trade.ResultRetcodeDescription());
   }
   if(ok1||ok2)
   {
...
bool ManageCandleStructureTrail()
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      {
         if(PrintExecutionLog)
            Print("NeoFL GOLD 5.2 FIX1 | CANDLE TRAIL FAILED | ",symbol,
                  " ticket=",ticket,
      ...
      else
      {
         any=true;
         if(PrintExecutionLog)
            Print("NeoFL GOLD 5.2 FIX1 | CANDLE TRAIL | ",symbol,
                  " ticket=",ticket,
...
bool ManageAllPositions()
{
   ...
   for(int i=PositionsTotal()-1;i>=0;i--)
   {
      ...
      {
         if(PrintExecutionLog)
            Print("NeoFL GOLD 5.2 FIX1 | ",reason," FAILED | ",symbol,
                  " ticket=",ticket,
                  " retcode=",trade.ResultRetcode(),
                  " ",trade.ResultRetcodeDescription(),
                  " oldSL=",DoubleToString(sl,DigitsOf(symbol)),
      ...
      {
         Print("NeoFL GOLD 5.2 FIX1 | ",reason," | ",symbol,
               " ticket=",ticket,
               " profitR=",DoubleToString(profitR,2),
...
void EvaluatePendingThesis()
{
   ...
   {
      if(PrintExecutionLog) Print("NeoFL GOLD 5.2 FIX1 | THESIS EXPIRED | ",PendingSymbol);
   ...
   {
      if(PrintExecutionLog) Print("NeoFL GOLD 5.2 FIX1 | THESIS INVALIDATED | ",PendingSymbol," trend=",DoubleToString(m.trend,2));
...
void FindAndExecuteBest()
{
   ...
   if(m.regime==MARKET_STRONG_UP || m.regime==MARKET_STRONG_DOWN)
   {
      ...
      {
         if(PrintExecutionLog)
            Print("NeoFL GOLD 5.2 FIX1 | M5 PRIMARY TREND GATE BLOCK | symbol=",m.symbol,
...
int OnInit()
{
   ...
   Print("NeoFL GOLD 5.2 FIX1 STARTED | "
         "DEDICATED ACTIVE SCANNER | symbols=",MarketCount,
         " | scan=",ScanSeconds,"s | host=",_Symbol);

   Print("NeoFL GOLD 5.2 FIX1 | Scanner reads only the "
                        ...
                        const MqlTradeResult &result)
{
      ...
      GoldBlockOppositeAfterStop)
   {
         ...
         (GoldShockEventBar>0 && TimeCurrent()-GoldShockEventBar<=GoldShockLockSeconds))
      {
         ...
         if(PrintExecutionLog)
            Print("NeoFL GOLD 5.2 FIX1 | STOP DURING/AT LIQUIDITY EVENT | ",
   ...
   if(PrintExecutionLog)
      Print("NeoFL GOLD 5.2 FIX1 | FILL DETECTED | ",symbol,
