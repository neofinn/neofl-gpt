"""NeoFL gateway HTTP transport with account Brain routing and native MT5."""
...
class GatewayHandler(BaseHTTPRequestHandler):
    server_version = "NeoFL-Gateway/1.8"
    api: ApiRegistry; webhooks: WebhookRegistry; store: StateStore; agent: AgentLoop; body: NeoFLBody; runtime: NeoFLAgentRuntime
    brains: BrainRegistry; telemetry: TelemetryRegistry; hermes: HermesAdapter; mt5_native: MT5NativeConnector
    ...
    def do_GET(self) -> None:
        ...
        if path == "/mt5/status":
            if not self._auth(): self.send_response(401); self.end_headers(); return
            self._send(200,{"accounts":self.telemetry.snapshot(),"brains":self.brains.snapshot(),"native":self.mt5_native.status()}); return
        if path == "/mt5/native/status":
            if not self._auth(): self.send_response(401); self.end_headers(); return
            try:
                self._send(200,{"native":self.mt5_native.status(),"account":self.mt5_native.account(),"terminal":self.mt5_native.terminal()})
            except MT5NativeError as exc: self._error(502,str(exc))
            return
        if path == "/mt5/native/account":
            if not self._auth(): self.send_response(401); self.end_headers(); return
            try: self._send(200,self.mt5_native.account())
            except MT5NativeError as exc: self._error(502,str(exc))
            return
        if path == "/mt5/native/symbol":
            if not self._auth(): self.send_response(401); self.end_headers(); return
            symbol=parse_qs(urlparse(self.path).query).get("symbol",[None])[0]
            if not symbol: self._error(400,"symbol is required"); return
            try: self._send(200,self.mt5_native.symbol(symbol))
            except MT5NativeError as exc: self._error(502,str(exc))
            return
        if path == "/mt5/native/tick":
            if not self._auth(): self.send_response(401); self.end_headers(); return
            symbol=parse_qs(urlparse(self.path).query).get("symbol",[None])[0]
            if not symbol: self._error(400,"symbol is required"); return
            try: self._send(200,self.mt5_native.tick(symbol))
            except MT5NativeError as exc: self._error(502,str(exc))
            return
        if path == "/mt5/native/positions":
            if not self._auth(): self.send_response(401); self.end_headers(); return
            symbol=parse_qs(urlparse(self.path).query).get("symbol",[None])[0]
            try: self._send(200,self.mt5_native.positions(symbol))
            except MT5NativeError as exc: self._error(502,str(exc))
            return
        if path == "/mt5/native/orders":
            if not self._auth(): self.send_response(401); self.end_headers(); return
            symbol=parse_qs(urlparse(self.path).query).get("symbol",[None])[0]
            try: self._send(200,self.mt5_native.orders(symbol))
            except MT5NativeError as exc: self._error(502,str(exc))
            return
        ...
        if path == "/agent/hermes/tools":
            if not self._auth(): self.send_response(401); self.end_headers(); return
            manifest=self.hermes.tool_manifest(); self._send(200,{"tools":manifest,"count":len(manifest)}); return
    ...
    def do_POST(self) -> None:
        ...
        if path == "/admin/brain/global-switch":
            if not self._auth(): self.send_response(401); self.end_headers(); return
            try:
                deployment=str(payload["brain"] or payload["deployment"]); resolved=self.brains.set_default(deployment)
        ...
        if path in {"/admin/brain/switch","/admin/brain/assign"}:
            if not self._auth(): self.send_response(401); self.end_headers(); return
            try:
                account=str(payload["account_id"]); deployment=str(payload["brain"] or payload["deployment"]); binding=self.brains.assign(account,deployment); resolved=self.brains.resolve(account)
        ...
        if path == "/admin/brain/use-global":
            if not self._auth(): self.send_response(401); self.end_headers(); return
            try:
                account=str(payload["account_id"]); self.brains.clear_assignment(account); resolved=self.brains.resolve(account); self._send(200,{"account_id":account,"routing":"GLOBAL","deployment":resolved.__dict__})
            except (KeyError,ValueError) as exc: self._error(400,str(exc))
            return
        if path == "/mt5/native/check-order":
            if not self._auth(): self.send_response(401); self.end_headers(); return
            try: self._send(200,{"checked":True,"result":self.mt5_native.check_order(payload.get("request") or {})})
            except MT5NativeError as exc: self._error(502,str(exc))
            return
        if path == "/mt5/native/send-order":
            if not self._auth(): self.send_response(401); self.end_headers(); return
            try: self._send(200,{"sent":True,"result":self.mt5_native.send_order(payload.get("request") or {})})
            except MT5NativeError as exc: self._error(403 if "disabled" in str(exc) else 502,str(exc))
            return
        if path == "/mt5/report":
            try:
                account=str(payload.get("account_id","")); deployment=self.brains.resolve(account); state=self.telemetry.heartbeat(payload,deployment.__dict__ if deployment else None); self._send(200,{"status":"CONNECTED","account":account,"brain":deployment.name if deployment else None,"branch":deployment.branch if deployment else None,"build":deployment.build if deployment else None,"mcp":state.get("mcp",{"status":"UNKNOWN"})})
            except ValueError as exc: self._error(400,str(exc))
            return
        if path == "/mt5/execution-report":
            try:
                account=str(payload.get("account_id","")); deployment=self.brains.resolve(account); state=self.telemetry.execution(payload); self._send(200,{"accepted":True,"account":account,"brain":deployment.name if deployment else None,"branch":deployment.branch if deployment else None,"build":deployment.build if deployment else None,"execution":state.get("last_execution_event")})
            except ValueError as exc: self._error(400,str(exc))
            return
        ...
        if path == "/agent/hermes/call":
            if not self._auth(): self.send_response(401); self.end_headers(); return
            try:
                name=str(payload["name"]); result=self.hermes.call(name,payload.get("arguments") or {}); self._send(200,{"ok":True,"tool":name,"result":result})
            except (KeyError,PermissionError,ValueError) as exc: self._error(400,str(exc))
            except Exception: log.exception("Hermes tool call failed"); self._error(502,"Hermes tool call failed")
            return
        if path == "/input":
            if not self._auth(): self.send_response(401); self.end_headers(); return
            try:
                request=AgentRequest(text=str(payload.get("text","")),symbol=payload.get("symbol"),mode=str(payload.get("mode","analyze")),request_id=payload.get("request_id"),context=payload.get("context") or {}); action=self.body.think(request); self._send(200 if action.allowed else 409,action.response)
        ...
        action=self.body.receive_external_event(source=f"webhook:{hook.name}",symbol=snapshot.mapped_symbol,payload=snapshot.to_dict(),quality=snapshot.quality.value); self._send(200,{"accepted":action.allowed,"symbol":snapshot.mapped_symbol,"soul":action.response})
...
def make_server(api,webhooks,store,agent,host="127.0.0.1",port=8787):
    body=NeoFLBody(agent,store); mcp=_mcp_client_from_environment(); runtime=NeoFLAgentRuntime(agent,mcp); hermes=HermesAdapter(mcp_client=mcp); mt5_native=MT5NativeConnector()
    brains=BrainRegistry([BrainDeployment("MAIN","main",os.getenv("NEOFL_MAIN_BUILD","unknown"),os.getenv("NEOFL_MAIN_BRAIN_URL","")),BrainDeployment("PARALLEL","neoflgpt-parallel",os.getenv("NEOFL_PARALLEL_BUILD","unknown"),os.getenv("NEOFL_PARALLEL_BRAIN_URL", ""))], default=os.getenv("NEOFL_DEFAULT_BRAIN","MAIN")); telemetry=TelemetryRegistry()
    handler=type("BoundGatewayHandler",(GatewayHandler,),{"api":api,"webhooks":webhooks,"store":store,"agent":agent,"body":body,"runtime":runtime,"brains":brains,"telemetry":telemetry,"hermes":hermes,"mt5_native":mt5_native})
